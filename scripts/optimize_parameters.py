...
def calculate_black_scholes_delta(s: float, k: float, t_days: float, sigma: float = 0.30, r: float = 0.05, option_type: str = "call") -> float:
...
def calculate_black_scholes_premium(s: float, k: float, t_days: float, sigma: float = 0.30, r: float = 0.05, option_type: str = "call") -> float:
...
def main():
    ...
    parser.add_argument("--asset", type=str, required=True, help="Asset class (equity, forex, futures, crypto, options)")
    parser.add_argument("--params_range", type=str, required=True, help="JSON string of parameter ranges")
    parser.add_argument("--data_path", type=str, default=None, help="Optional data path override")
    parser.add_argument("--metric", type=str, default="sharpe", choices=["sharpe", "max_drawdown", "profit_factor", "total_return", "win_rate"], help="Metric to optimize")
    parser.add_argument("--out", type=str, default="logs/optimization_results.json", help="Output path")
    ...
    try:
        spec = importlib.util.spec_from_file_location(f"strategies.custom_{args.asset.lower()}", strategy_path)
    ...
    for i, combination in enumerate(grid):
        ...
        for idx, row in df.iterrows():
            ...
            if args.asset.lower() == "options":
                ...
                opt_premium = (bid + ask) / 2.0 if (bid > 0 and ask > 0) else calculate_black_scholes_premium(ref_price, strike_price, float(row.get("dte", 30.0)))
                portfolio_delta = calculate_black_scholes_delta(ref_price, strike_price, float(row.get("dte", 30.0)), option_type=str(row.get("option_type", "call")))
            ...
            for o in orders:
                ...
                if args.asset.lower() == "options" and getattr(o, "client_order_id", "") == "covered_call_write":
                    ...
                    if order_sym in active_positions:
                        ...
                        if entry["qty"] > 0:
                            ...
                            trades.append({
                                ...
                                "exit_ts": ts,
                                "pnl": pnl,
                                "pnl_pct": pnl / (entry["price"] * entry["qty"]) if entry["price"] > 0 else 0
                            })
                            entry["qty"] -= qty
                            if entry["qty"] <= 0:
        ...
        if len(equity_series) > 1:
            ...
            profit_factor = float(sum(wins) / sum(losses)) if (losses and sum(losses) > 0) else (999.0 if wins else 1.0)
            recovery_factor = float(total_return / max_drawdown) if max_drawdown > 0 else total_return
        else:
            total_return = 0.0
            sharpe = 0.0
            max_drawdown = 0.0
    ...
    try:
        ...
        print(f"Best parameters: {best_comb.get('parameters')} -> Sharpe: {best_comb.get('metrics', {}).get('sharpe')}, MaxDD: {best_comb.get('metrics', {}).get('max_drawdown')}%")
    except Exception as e:
