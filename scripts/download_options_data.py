...
def generate_synthetic_options(underlying: str, start_date: str, end_date: str, strike_range: str, include_greeks: bool, output_dir: str, dte_range: str = "all"):
    ...
    for d in dates:
        ...
        for exp in expirations:
            ...
            strikes = [atm_strike + i * strike_increment for i in range(-num_strikes // 2, num_strikes // 2 + 1)]
            ...
            for K in strikes:
                ...
                for option_type in ["call", "put"]:
                    ...
                    if option_type == "call":
                        ...
                        delta = nd1
                        theta = -(spot * norm_pdf(d1) * sigma) / (2 * math.sqrt(t)) - r * K * math.exp(-r * t) * nd2
                    else:
                        price = K * math.exp(-r * t) * n_minus_d2 - spot * n_minus_d1
                        delta = nd1 - 1.0
                        theta = -(spot * norm_pdf(d1) * sigma) / (2 * math.sqrt(t)) + r * K * math.exp(-r * t) * n_minus_d2
                    ...
                    occ_symbol = f"{underlying.upper().ljust(6)}{date_str}{type_char}{strike_str}".replace(" ", "")
...
def download_theta_data(underlying: str, start_date: str, end_date: str, include_greeks: bool, output_dir: str, config: dict, dte_range: str = "all"):
    ...
    try:
        ...
        print(f"Downloading options for {underlying} from {start_date} to {end_date} using ThetaData...")
...
def main():
    ...
    parser.add_argument("--include-greeks", action="store_true", help="Calculate and include Greeks")
