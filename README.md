...
> [!IMPORTANT]
> Все тренировочные пайплайны должны включать PopArt‑контроллер (см. блок
> выше). Без него в мониторинг не попадут метрики `config/popart/*`,
> `shadow_popart/*`, `gate/*` и `popart/apply_count`, что усложняет диагностику
> и повторно приводит к деградациям головы ценности.
