...
> [!NOTE]
> В базовых тренировочных конфигурациях PopArt выключен, поэтому метрики
> `config/popart/*`, `shadow_popart/*`, `gate/*` и `popart/apply_count`
> недоступны. Подключайте контроллер, когда требуется диагностика и защита
> головы ценности на удержанном батче.
