...
class ObsParams:
    ...
    def n_features(self) -> int:
        """Базовая оценка длины вектора признаков (N_FEATURES) для legacy-модели.

        Расчёт основан на фиксированных группах признаков в симуляторе и не
        подтягивается автоматически из актуального пайплайна. При изменении
        набора признаков значения ниже необходимо синхронизировать вручную,
        иначе цифра будет вводить в заблуждение.
        """
        base_count = 3
        indicator_count = 13
        derived_count = 2
