...
def main() -> int:
    ...
    payload = {
        "data": {
            "prices": "yahoo (free)",
            "fundamentals": "SEC EDGAR XBRL (free, PIT=filing date)",
            "universe": base["universe"]["symbols"],
            "start": d0,
            "end": d1,
            "n_symbols": nsym,
        },
        "pit_provenance": pit_rows,
        "data_trust_verdict": verdict,
        "summary": s,
        "trust_report": t,
    }
...
def _write_md(p):
    ...
    L.append(
        "> **Честный point-in-time.** Цены — Yahoo (free). Фундаментал — SEC EDGAR XBRL "
    ...
    L.append("## Вывод\n")
    L.append(
        "- Все колонки сигналов имеют **pit_quality=true** (включая EDGAR-фундаментал) → "
        "бэктест **backtest-safe**, value/quality честны."
    )
    L.append(
        "- Платные Sharadar/Compustat подключаются через тот же `fundamentals_path` parquet "
    ...
    L.append(
        "- Survivorship-free юниверс: `universe.type: index_membership` + `membership_path` "
...
def _track(base, payload, panel):
    ...
    try:
        ...
        d = payload["data"]
        s = payload["summary"]
        t = payload["trust_report"]
        with tr.run(
            "xs_equity_real",
            params={
                "universe_size": len(d["universe"]),
                "data_trust": payload["data_trust_verdict"],
            },
            tags={"asset_class": "equity", "pit": "true"},
        ) as run:
            run.set_lineage(
                dataset_uri="yahoo+sec_edgar:" + ",".join(d["universe"]),
                config_uri=CONFIG,
                config_hash=hash_config(base),
            ...
            run.log_artifact(OUT_JSON)
            run.log_artifact(OUT_MD)
        ...
        mv = reg.register(
            ...
            lineage=lineage,
            description="Real-data equity XS w/ EDGAR PIT fundamentals.",
        )
        return {
            "run_id": rid,
            "model": "xs_equity_alpha",
            "version": mv.version,
            "artifact_algo": mv.artifact.algo,
            "signature_valid": reg.verify("xs_equity_alpha", mv.version),
