...
def main(argv=None) -> int:
    ...
    if a.cmd == "experiments":
        ...
        _p(
            [
                {"run_id": r.run_id, "status": r.status, "metrics": r.metrics}
                for r in t.list_runs(a.experiment)
            ]
        )
    elif a.cmd == "run":
        ...
        names = (
            sorted([d for d in os.listdir(reg.root) if os.path.isdir(os.path.join(reg.root, d))])
        ...
        for n in names:
            ...
            out.append(
                {
                    "name": n,
                    "n_versions": len(reg.list_versions(n)),
        ...
        _p(
            [
                {**v.to_dict(), "signature_valid": reg.verify(a.model, v.version)}
        ...
        _p(
            {
                "model": a.model,
                "version": a.version,
                "signature_valid": reg.verify(a.model, a.version),
            }
        )
    return 0
