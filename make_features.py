...
def main():
    p = argparse.ArgumentParser(
        description="Build offline features using the same transformers as realtime."
    )
    p.add_argument(
        "--in",
        dest="in_path",
        required=True,
        help="Вход: parquet/csv с колонками ts_ms,symbol,price (обычно prices.parquet)",
    )
    p.add_argument(
        "--out", dest="out_path", default="data/features.parquet", help="Куда сохранить parquet фич"
    )
    p.add_argument(
        "--price-col", default="price", help="Имя колонки с ценой (по умолчанию 'price')"
    )
    p.add_argument(
        "--lookbacks",
        default="240,720,1200,1440,5040,10080,12000",
        help="Окна SMA/ret в минутах для 4h интервала (по умолчанию 240,720,1200,1440,5040,10080,12000 = 4h,12h,20h,24h,3.5d,7d,200h)",
    )
    p.add_argument("--rsi-period", type=int, default=14, help="Период RSI (Wilder)")
    p.add_argument(
        "--yang-zhang-windows",
        default="2880,10080,43200",
        help="Окна Yang-Zhang в минутах для 4h (по умолчанию 2880,10080,43200 = 48h,7d,30d)",
    )
    ...
    p.add_argument(
        "--taker-buy-ratio-windows",
        default="480,960,1440",
        help="Окна Taker Buy Ratio SMA в минутах для 4h (по умолчанию 480,960,1440 = 8h,16h,24h)",
    )
    p.add_argument(
        "--taker-buy-ratio-momentum",
        default="240,480,720,1440",
        help="Окна Taker Buy Ratio momentum в минутах для 4h (по умолчанию 240,480,720,1440 = 4h,8h,12h,24h)",
    )
    p.add_argument(
        "--volume-col", default=None, help="Имя колонки volume для Taker Buy Ratio (опционально)"
    )
    p.add_argument(
        "--taker-buy-base-col",
        default=None,
        help="Имя колонки taker_buy_base для Taker Buy Ratio (опционально)",
    )
    p.add_argument(
        "--cvd-windows",
        default="1440,10080",
        help="Окна Cumulative Volume Delta в минутах для 4h (по умолчанию 1440,10080 = 24h,7d)",
    )
    p.add_argument(
        "--parkinson-windows",
        default="2880,10080",
        help="Окна Parkinson волатильности в минутах для 4h (по умолчанию 2880,10080 = 48h,7d)",
    )
    p.add_argument(
        "--garch-windows",
        default="12000,20160,43200",
        help="Окна GARCH(1,1) волатильности в минутах для 4h (по умолчанию 12000,20160,43200 = 200h/8.3d, 14d, 30d)",
    )
    p.add_argument(
        "--bar-duration-minutes",
        type=int,
        default=240,
        help="Длительность одного бара в минутах (по умолчанию 240 для 4h интервала)",
    )
    p.add_argument(
        "--selected-features",
        default=None,
        help="Список выбранных колонок фич через запятую. Если опущен, сохраняются все рассчитанные фичи.",
    )
    args = p.parse_args()
    ...
    if "ts_ms" not in df.columns:
        ...
        if ts_candidate is not None:
            ...
            raise SystemExit(
                "Ошибка: во входных данных отсутствует временная колонка (ts_ms или timestamp)"
            )
    ...
    taker_buy_ratio_wins = [
        int(s.strip()) for s in str(args.taker_buy_ratio_windows).split(",") if s.strip()
    ]
    taker_buy_ratio_mom = [
        int(s.strip()) for s in str(args.taker_buy_ratio_momentum).split(",") if s.strip()
    ]
    ...
    taker_buy_base_col = (
        args.taker_buy_base_col
        if args.taker_buy_base_col and args.taker_buy_base_col in df.columns
        else None
    )
    ...
    if args.selected_features is not None:
        ...
        if selected_set:
            ...
            keep_cols = [
                col for col in feats.columns if col in mandatory_cols or col in selected_set
            ]
