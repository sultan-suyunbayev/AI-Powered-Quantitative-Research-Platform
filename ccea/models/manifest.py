...
class ModelRef(BaseModel):
    ...
    digest: str = Field(
        ...
        description="SHA256 digest of model file"
    )
...
class DataContract(BaseModel):
    ...
    output_schema: Optional[Dict[str, Any]] = Field(
        None, description="Output data schema"
    )
...
class RiskProfileSuggested(BaseModel):
    ...
    max_position_pct: Optional[float] = Field(
        None, ge=0, le=100,
        description="Suggested max position as % of portfolio"
    )
    max_daily_loss_pct: Optional[float] = Field(
        None, ge=0, le=100,
        description="Suggested max daily loss as % of portfolio"
    )
    max_orders_per_minute: Optional[int] = Field(
        None, ge=1,
        description="Suggested max order rate"
    )
...
class ArtifactManifest(BaseModel):
    ...
    deps_lock_digest: str = Field(
        ...,
        pattern=r"^sha256:[a-f0-9]{64}$",
        description="SHA256 digest of locked dependencies"
    )
    created_at: datetime
    ...
    telemetry_schema_version: Optional[str] = Field(
        None,
        pattern=r"^[0-9]+\.[0-9]+\.[0-9]+$"
    )
