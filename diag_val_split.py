...
def detect_splits(
    ...
    for key in ("splits", "dataset_splits"):
        if key in off and isinstance(off[key], dict) and "time" in off[key]:
            timeS = off[key]["time"]
            t = (
                to_ts(timeS.get("train", {}).get("start")),
                to_ts(timeS.get("train", {}).get("end")),
            )
...
def main():
    ...
    result = {
        ...
        "train_window": {
            "start": train_win[0],
            "end": train_win[1],
            "start_utc": pretty(train_win[0]),
            "end_utc": pretty(train_win[1]),
        },
        "val_window": {
            "start": val_win[0],
            "end": val_win[1],
            "start_utc": pretty(val_win[0]),
            "end_utc": pretty(val_win[1]),
        },
    ...
    if src == "not_found":
        result["notes"].append(
            "❗ Сплиты не найдены ни в config.data.*, ни в data.split_path, ни в configs/offline.yaml."
        )
    if val_cov == 0:
        result["notes"].append(
            "❗ Ни один символ не пересекается с валид. окном — как в твоей ошибке ValueError."
        )
    ...
    # Сводная по символам
    sym = (
        df.groupby("symbol")[["ts_min", "ts_max", "rows"]]
        .agg({"ts_min": "min", "ts_max": "max", "rows": "sum"})
        .reset_index()
        if not df.empty
        else pd.DataFrame(columns=["symbol", "ts_min", "ts_max", "rows"])
    )
    ...
    sym["covers_train"] = sym.apply(
        lambda r: (
            int(not (r.ts_max < train_win[0] or r.ts_min > train_win[1]))
            if train_win[0] and train_win[1] and pd.notna(r.ts_min) and pd.notna(r.ts_max)
            else 0
        ),
        axis=1,
    )
    sym["covers_val"] = sym.apply(
        lambda r: (
            int(not (r.ts_max < val_win[0] or r.ts_min > val_win[1]))
            if val_win[0] and val_win[1] and pd.notna(r.ts_min) and pd.notna(r.ts_max)
            else 0
        ),
        axis=1,
    )
    sym.sort_values(
        ["covers_val", "covers_train", "rows"], ascending=[False, False, False], inplace=True
    )
