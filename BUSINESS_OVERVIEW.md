...
**Hard rule (использовать последовательно в документах):** Cloud **не хранит** broker credentials и **не отправляет live trading instructions** (orders/targets/signals). Любое live‑исполнение возможно только через customer‑controlled Agent и аккаунты клиента у брокера.
