...
| **Риск-менеджмент** | ✅ гварды по всем классам, kill switch, pre-trade VaR | ✅ Kill switch, asset-гварды, pre-trade VaR/сценарии — код+тесты реальные. 🔴 **Enforcement-разрыв:** из формы риск-лимитов Lite реально enforced только `max_total_notional`/`max_total_exposure_pct`/`exposure_buffer_frac` (читает `service_signal_runner.py:9469`); `lite_limits` (daily loss, max DD, leverage, PDT/SPAN/Greeks-тумблеры) **никем в runtime не читаются**. |
...
6. 🔴 `lite_limits` из configs/risk.yaml (daily loss, max DD, leverage, PDT/SPAN/Greeks) сохраняются, но **не enforced ни одним runtime-компонентом** (endpoint честно говорит `applied_to_agent: false`, однако и «следующий RUN» их не подхватит — потребителя нет).
...
| P0-B | **Enforcement риск-лимитов**: заставить `service_signal_runner`/agentd читать `lite_limits` (daily loss → стоп торговли, max DD → стоп, leverage cap); либо честно убрать поля из формы | §3.6 |
