    ...
    def get_metrics_with_drift(self) -> List[MonitoringMetric]:
        ...
        with self._lock:
            return [m for m in self._metrics.values()
                    if m.drift_severity != DriftSeverity.NONE]
...
class IncidentTracker:
    ...
    def get_serious_incidents(self) -> List[Incident]:
        ...
        with self._lock:
            return [i for i in self._incidents.values()
                    if i.severity == IncidentSeverity.SERIOUS]
...
class PostMarketMonitoringSystem:
    ...
    def register_metric(
        ...
        self._log_event("metric_registered", {
            ...
            if drift:
                ...
                if drift.severity.value >= DriftSeverity.MEDIUM.value:
                    self._generate_alert(
                        priority=AlertPriority.HIGH if drift.severity == DriftSeverity.CRITICAL else AlertPriority.WARNING,
    ...
    def report_incident(
        ...
        if severity.value >= IncidentSeverity.MAJOR.value:
            self._generate_alert(
                priority=AlertPriority.CRITICAL if severity == IncidentSeverity.SERIOUS else AlertPriority.HIGH,
    ...
    def update_incident(
        ...
        self._log_event("incident_updated", {
    ...
    def set_incident_root_cause(
        ...
        return self._incident_tracker.set_root_cause(
            incident_id, root_cause, contributing_factors
        )
    ...
    def add_incident_action(
        ...
        return self._incident_tracker.add_corrective_action(
            incident_id, action, action_type
        )
    ...
    def submit_regulatory_report(
        ...
        self._log_event("regulatory_report_submitted", {
            "incident_id": incident_id,
            "authority": authority,
        })
        return incident
    ...
    def generate_periodic_report(
        ...
        # Calculate metrics
        accuracy_values = [
            m.current_value for m in metrics
        ...
        report = PeriodicReport(
            ...
            serious_incidents=len([i for i in incidents.values()
                                  if i.severity == IncidentSeverity.SERIOUS]),
            ...
            critical_drifts=len([d for d in drifts.values()
                                if d.severity == DriftSeverity.CRITICAL]),
            total_alerts=len(alerts),
            critical_alerts=len([a for a in alerts.values()
    ...
    def _generate_recommendations(self, report: PeriodicReport) -> List[str]:
        ...
        pending_reports = self.get_pending_regulatory_reports()
        if pending_reports:
            recommendations.append(f"Submit {len(pending_reports)} pending regulatory report(s) within required timeframe")
    ...
    def get_compliance_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "drift": {
                ...
                "critical": len([d for d in self._drift_detections.values()
                                if d.severity == DriftSeverity.CRITICAL]),
            },
            "incidents": {
            ...
            "alerts": {
                ...
                "critical": len([a for a in active_alerts
                                if a.priority == AlertPriority.CRITICAL]),
            },
            "feedback": {
            ...
            "is_compliant": len(pending_reports) == 0 and len([
