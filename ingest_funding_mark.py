    ...
    *,
    ...
    while cur < end_ms:
        batch = client.get_mark_klines(
            symbol=symbol, interval=interval, start_ms=cur, end_ms=end_ms, limit=limit
        )
        if not batch:
            cur += 60_000
            time.sleep(sleep_ms / 1000.0)
...
def main():
    p = argparse.ArgumentParser(
        description="Ingest Binance Futures funding and mark-price klines (public)."
    )
    ...
    p.add_argument(
        "--mark-interval", default="4h", help="Интервал mark-price klines (изменено с 1m на 4h)"
    )
