...
# - Это обеспечивает баланс: гибкость генерации + компактность observation (56 признаков)
# - Аналогично: ret_7d генерируется, но не используется в observation
# - НЕ ИЗМЕНЯТЬ без переобучения моделей (они ожидают ровно 56 признаков)
...
def get_feature_spec_4h():
    ...
    # - observation vector остается компактным (56 признаков) для эффективности
