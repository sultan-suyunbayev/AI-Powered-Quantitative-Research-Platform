...
_SETTLE_DAYS = {"equity": 1, "etf": 1, "option": 1, "fx": 2, "crypto": 0,
                "future": 0, "index": 0}
...
class TradeBlotter:
    ...
    def _head_hash(self) -> str:
        if self._conn is not None:
            row = self._conn.execute("SELECT entry_hash FROM trades ORDER BY seq DESC LIMIT 1").fetchone()
    ...
    def record_trade(
        self, *, symbol: str, side: str, quantity: Any, price: Any,
        fee: Any = 0, financing: Any = 0, figi: Optional[str] = None,
        currency: str = "USD", asset_class: str = "equity", strategy_id: str = "",
        client_order_id: Optional[str] = None, broker_order_id: Optional[str] = None,
        ts: Optional[datetime] = None,
    ) -> TradeRecord:
        with self._lock:
            t = ts or _now()
            seq = self._count + 1
            qty = _f(quantity); px = _f(price)
            payload = {
                "ts": t.isoformat(), "symbol": str(symbol), "figi": figi,
                "side": str(side).lower(), "quantity": qty, "price": px,
                "gross_notional": abs(qty * px), "fee": _f(fee), "financing": _f(financing),
                "currency": currency, "asset_class": asset_class, "strategy_id": strategy_id,
            ...
            if self._conn is not None:
                self._conn.execute(
                    """INSERT INTO trades(ts,symbol,figi,side,quantity,price,gross_notional,fee,
                       ...
                       VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (rec.ts, rec.symbol, rec.figi, rec.side, str(rec.quantity), str(rec.price),
                     str(rec.gross_notional), str(rec.fee), str(rec.financing), rec.currency,
                     rec.asset_class, rec.strategy_id, rec.client_order_id, rec.broker_order_id,
                     rec.settlement_date, rec.prev_hash, rec.entry_hash),
                )
                self._conn.commit()
            else:
        ...
        if self._conn is not None:
            ...
            for r in rows:
                out.append(TradeRecord(
                    seq=r["seq"], ts=r["ts"], symbol=r["symbol"], figi=r["figi"], side=r["side"],
                    quantity=float(r["quantity"]), price=float(r["price"]),
                    gross_notional=float(r["gross_notional"]), fee=float(r["fee"]),
                    financing=float(r["financing"]), currency=r["currency"],
                    asset_class=r["asset_class"], strategy_id=r["strategy_id"],
                    client_order_id=r["client_order_id"], broker_order_id=r["broker_order_id"],
                    settlement_date=r["settlement_date"], prev_hash=r["prev_hash"],
    ...
    def verify(self) -> Dict[str, Any]:
        recs = self._all_records()
        chain = [ChainRecord(seq=r.seq, payload=_blotter_payload(r), prev_hash=r.prev_hash,
                             entry_hash=r.entry_hash) for r in recs]
...
def _blotter_payload(r: TradeRecord) -> Dict[str, Any]:
    return {
        "ts": r.ts, "symbol": r.symbol, "figi": r.figi, "side": r.side,
        "quantity": r.quantity, "price": r.price, "gross_notional": r.gross_notional,
        "fee": r.fee, "financing": r.financing, "currency": r.currency,
        "asset_class": r.asset_class, "strategy_id": r.strategy_id,
        "client_order_id": r.client_order_id, "broker_order_id": r.broker_order_id,
...
class CashLedger:
    ...
    def __init__(self, db_path: Optional[Path] = None, *, opening_balance: float = 0.0,
                 currency: str = "USD", hmac_key: Optional[bytes] = None) -> None:
        self._lock = threading.RLock()
        self._key = hmac_key
        ...
        if self._db_path is not None:
            ...
            self._conn.execute("CREATE TABLE IF NOT EXISTS cash_meta (key TEXT PRIMARY KEY, value TEXT)")
            row = self._conn.execute("SELECT value FROM cash_meta WHERE key='opening'").fetchone()
            if row is None:
                self._conn.execute("INSERT INTO cash_meta(key,value) VALUES('opening',?)", (str(self._opening),))
                self._conn.commit()
            else:
    ...
    def _last(self) -> Optional[CashMovement]:
        if self._conn is not None:
            r = self._conn.execute("SELECT * FROM cash_movements ORDER BY seq DESC LIMIT 1").fetchone()
            if r is None:
                return None
            return CashMovement(seq=r["seq"], ts=r["ts"], type=r["type"], amount=float(r["amount"]),
                                balance=float(r["balance"]), currency=r["currency"], ref=r["ref"],
                                symbol=r["symbol"], prev_hash=r["prev_hash"], entry_hash=r["entry_hash"])
    ...
    def post(self, type_: str, amount: Any, *, ref: Optional[str] = None,
             symbol: Optional[str] = None, ts: Optional[datetime] = None) -> CashMovement:
        t = str(type_).upper()
        ...
        with self._lock:
            ...
            payload = {"ts": tt, "type": t, "amount": _f(amount), "balance": round(bal, 10),
                       "currency": self._currency, "ref": ref, "symbol": symbol}
            h = chain_hash(prev, payload, seq, key=self._key)
            ...
            if self._conn is not None:
                self._conn.execute(
                    """INSERT INTO cash_movements(ts,type,amount,balance,currency,ref,symbol,prev_hash,entry_hash)
                       VALUES(?,?,?,?,?,?,?,?,?)""",
                    (mv.ts, mv.type, str(mv.amount), str(mv.balance), mv.currency, mv.ref,
                     mv.symbol, mv.prev_hash, mv.entry_hash))
                self._conn.commit()
            else:
                self._mem.append(mv)
            return mv
    ...
    def verify(self) -> Dict[str, Any]:
        recs = self._all()
        chain = [ChainRecord(seq=m.seq, payload={"ts": m.ts, "type": m.type, "amount": m.amount,
                             "balance": m.balance, "currency": m.currency, "ref": m.ref,
                             "symbol": m.symbol}, prev_hash=m.prev_hash, entry_hash=m.entry_hash)
                 for m in recs]
        out = verify_chain(chain, key=self._key)
        out["keyed"] = self._key is not None
        return out
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {"opening": self._opening, "balance": round(self.balance, 2),
                "n_movements": len(recs), "by_type": {k: round(v, 2) for k, v in by_type.items()},
                "integrity": self.verify()}
