...
class _IBBackend:
    ...
    def connect(self) -> bool:
        ...
        if not bool(self._a.connect()):
            raise ConnectionError(getattr(self._a, "last_error", None) or "IB connect returned false")
    ...
    def positions(self) -> List[Dict[str, Any]]:
        ...
        try:
            for p in self._a.get_positions() or []:
                out.append({"symbol": getattr(p, "symbol", ""), "qty": float(getattr(p, "qty", 0) or 0),
...
class IBConnector(DelegatingConnector):
    ...
    def __init__(self, credentials, *, sandbox: bool = True, config: Optional[Dict[str, Any]] = None,
        ...
        self._config = config or {"host": "127.0.0.1", "port": 7497 if sandbox else 7496, "client_id": 7}
