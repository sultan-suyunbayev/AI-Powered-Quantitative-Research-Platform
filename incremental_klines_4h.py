...
Adapted from incremental_klines.py for 4h timeframe.
This module backfills every missing candle since the beginning of Binance history
(or since the last recorded bar) and writes them to ``data/klines_4h/{SYMBOL}.csv``
with the original 12 Binance fields plus ``symbol``.

CLI:
  python incremental_klines_4h.py --symbols BTCUSDT,ETHUSDT [--close-lag-ms 8000]
  python incremental_klines_4h.py                       # load symbols from data/universe/symbols.json
"""
...
OUT_DIR = os.path.join("data", "klines_4h")
os.makedirs(OUT_DIR, exist_ok=True)

INTERVAL_MS = 14_400_000  # 4h = 4 * 3600 * 1000
MAX_BATCH = 1000
...
def sync_symbol(symbol: str, close_lag_ms: int, *, out_dir: Optional[str] = None) -> int:
    ...
    out_dir : str
        Directory where the CSV file is stored. Defaults to ``data/klines_4h``.
...
def run_many(symbols: List[str], close_lag_ms: int) -> int:
    ...
    print(f"✓ Appended {appended} closed 4h bars.")
    return appended
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=8000,
        help="Allowed lag in ms when verifying if the fetched bar is closed (default: 8000 for 4h)",
    )
