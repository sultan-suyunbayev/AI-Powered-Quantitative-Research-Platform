...
from service_optimizer import (
    OptimizerConstraints, PortfolioOptimizer, SizingConfig, TCostModel, kelly_weights,
)
...
def test_tcost_reduces_turnover_vs_no_tcost():
    ...
    opt1 = PortfolioOptimizer(objective="mean_variance", risk_aversion=5.0, constraints=cons,
...
def test_tcost_zero_recovers_unconstrained_direction():
    ...
    opt = PortfolioOptimizer(objective="mean_variance", constraints=cons,
...
def test_factor_cap_constraint_in_scipy():
    ...
    cons = OptimizerConstraints(gross_max=1.0, max_position=0.5,
                                exposures=B, factor_caps={"market": 0.1})
    opt = PortfolioOptimizer(objective="mean_variance", constraints=cons,
                             tcost=TCostModel(linear=0.001))
    w = opt.solve(mu, cov, current_w=pd.Series(0.0, index=syms))
