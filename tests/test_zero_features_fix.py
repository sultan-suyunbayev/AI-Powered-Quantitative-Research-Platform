...
class TestZeroFeaturesServiceTrain:
    ...
    def test_zero_features_raises_value_error(self, caplog):
        ...
        service = ServiceTrain(feature_pipe=MagicMock(), trainer=MagicMock(), cfg=mock_config)
        ...
        assert any(
            "Design matrix не содержит признаков" in record.message for record in caplog.records
        )
    ...
    def test_normal_features_works(self, caplog):
        ...
        try:
            ...
            assert (
                service is not None
            ), "Service should remain valid after processing normal features"
        except ZeroDivisionError:
...
class TestZeroFeaturesTrainScript:
    ...
    def test_zero_features_warning_logged(self, caplog):
        ...
        df = pd.DataFrame(
            {"timestamp": [1, 2, 3, 4, 5], "symbol": ["BTCUSDT"] * 5, "train_test": ["train"] * 5}
        )
        ...
        assert any(
            "нет признаков после исключения служебных колонок" in record.message
    ...
    def test_zero_features_shows_debug_info(self, caplog):
        ...
        df = pd.DataFrame(
            {"timestamp": [1, 2, 3], "symbol": ["ETHUSDT"] * 3, "train_test": ["train"] * 3}
        )
    ...
    def test_mixed_symbols_some_empty(self, caplog):
        ...
        df1 = pd.DataFrame(
            {"timestamp": [1, 2, 3], "symbol": ["SYM1"] * 3, "train_test": ["train"] * 3}
        )
