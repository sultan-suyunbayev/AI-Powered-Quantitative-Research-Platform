...
def test_leverage_skipped_for_exit():
    ...
    res = rc.check(
        _entry(itype=IntentType.CLOSE_POSITION, side=IntentSide.SHORT), pf, price=Decimal("10000")
    )
...
def test_build_risk_checker_maps_limits():
    lim = LiveRiskLimits(
        daily_loss_limit_usd=1000, max_leverage=2.0, max_drawdown_pct=15, max_concentration_pct=25
    )
...
def test_monitor_daily_loss_breach_triggers_halt(tmp_path):
    halts = []
    m = LiveRiskMonitor(
        limits_loader=_limits(daily_loss_limit_usd=500),
...
def test_monitor_drawdown_breach_triggers_halt(tmp_path):
    halts = []
    m = LiveRiskMonitor(
        limits_loader=_limits(max_drawdown_pct=10),
        halt_callback=lambda p: halts.append(p),
        peak_state_path=str(tmp_path / "peak.json"),
    )
...
def test_monitor_usage_percentages(tmp_path):
    m = LiveRiskMonitor(
        limits_loader=_limits(daily_loss_limit_usd=1000, max_leverage=4.0),
        peak_state_path=str(tmp_path / "peak.json"),
    )
