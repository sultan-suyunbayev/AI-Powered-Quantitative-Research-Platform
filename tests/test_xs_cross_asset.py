...
def test_currency_base_passthrough():
    ...
    out = normalize_returns_to_base(
        rw,
        currency_map={"SPY": "USD"},
        fx_returns={"EUR": pd.Series([0.5, 0.5], index=[0, 1])},
        base="USD",
    )
...
def test_joint_cov_psd_and_symmetric():
    b1 = AssetClassBlock(
        "crypto", pd.Series({"BTC": 0.6, "ETH": -0.4}), _rw(["BTC", "ETH"], 120, 0.03, 1)
    )
    b2 = AssetClassBlock(
        "equity", pd.Series({"AAPL": 0.5, "XOM": -0.5}), _rw(["AAPL", "XOM"], 120, 0.01, 2)
    )
...
def test_combine_hits_vol_target_and_spans_classes():
    b1 = AssetClassBlock(
        "crypto", pd.Series({"BTC": 0.6, "ETH": -0.4}), _rw(["BTC", "ETH"], 150, 0.03, 3)
    )
    b2 = AssetClassBlock(
        "equity", pd.Series({"AAPL": 0.5, "XOM": -0.5}), _rw(["AAPL", "XOM"], 150, 0.012, 4)
    )
...
def test_risk_parity_favors_low_vol_class():
    low = AssetClassBlock(
        "equity", pd.Series({"A": 0.5, "B": -0.5}), _rw(["A", "B"], 150, 0.005, 5)
    )
    high = AssetClassBlock(
        "crypto", pd.Series({"C": 0.5, "D": -0.5}), _rw(["C", "D"], 150, 0.05, 6)
    )
...
def test_block_from_config_and_combine():
    ...
    res = combine_from_configs(
        {"crypto": crypto, "equity": equity}, target_vol=0.12, class_weighting="risk_parity"
    )
    assert sum(res.class_allocations.values()) == pytest.approx(1.0)
...
def test_api_cross_asset_demo():
    ...
    r = client.post(
        "/api/xs/cross_asset",
        json={"demo": True, "target_vol": 0.10, "class_weighting": "risk_parity"},
    )
