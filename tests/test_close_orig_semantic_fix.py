...
class TestFetchAllDataPatchNoCloseOrig:
    ...
    def test_load_all_data_no_close_orig(self, tmp_path):
        ...
        assert loaded_df["close"].iloc[0] == 100.5, "close should not be shifted in load_all_data()"
...
class TestFeaturesPipelineShiftingWithoutCloseOrig:
    ...
    def test_fit_shifts_all_features_without_close_orig(self):
        ...
        assert 56.0 <= rsi_mean <= 59.0, f"rsi_14 mean {rsi_mean} suggests data was NOT shifted"
    ...
    def test_close_shifted_marker_created(self):
        ...
        assert (
            "_close_shifted" in transformed.columns
        ), "_close_shifted marker should be created after shifting"

        # All values should be True
        assert transformed["_close_shifted"].all(), "_close_shifted should be True for all rows"
...
class TestFeaturesPipelineWithCloseOrig:
    ...
    def test_transform_skips_shift_when_close_orig_present(self):
        ...
        assert not pd.isna(
            transformed["close"].iloc[0]
        ), "close should NOT be shifted when close_orig present"
        assert (
            transformed["close"].iloc[0] == 103.0
...
class TestTradingEnvCompatibility:
    ...
    def test_close_shifted_in_metadata_columns(self):
        ...
        assert "_close_shifted" in METADATA_COLUMNS, "_close_shifted should be in METADATA_COLUMNS"
    ...
    def test_close_shifted_not_in_columns_to_shift(self):
        ...
        assert "_close_shifted" not in cols, "_close_shifted should NOT be in columns_to_shift"
...
class TestBackwardCompatibility:
    ...
    def test_legacy_artifacts_without_preserve_close_orig(self):
        ...
        legacy_json = {
            "stats": {"close": {"mean": 100.0, "std": 5.0, "is_constant": False}},
            "metadata": {},
            "config": {
        ...
        try:
            ...
            assert (
                pipe.preserve_close_orig is True
