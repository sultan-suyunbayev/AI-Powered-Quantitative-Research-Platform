...
class TestScriptLiveNotRecommended:
        ...
        if "script_live.py" in content:
            ...
            for i, line in enumerate(lines):
                if "script_live.py" in line.lower():
                    ...
                    has_warning = any(word in context for word in [
                        "development", "testing", "dry-run", "legacy", "not production"
                    ])
                    # At least one mention should have warning context
                    if has_warning:
                        return
            # If we get here, check for production Agent mention
            assert "packages.agent" in content.lower() or "agentd" in content.lower(), \
    ...
    def test_architecture_has_ccea_note(self, project_root):
        ...
        assert "ccea" in content.lower() or "agent zone" in content.lower(), \
    ...
    def test_getting_started_ccea_compliant(self, project_root):
        ...
        if "live trading" in content.lower() or "live execution" in content.lower():
            has_ccea_context = any(term in content.lower() for term in [
                "agent", "ccea", "local execution", "local vault"
            ])
    ...
    def test_script_live_not_sole_path(self, project_root):
        ...
        for md_file in script_live_mentions:
            ...
            has_ccea_context = any(term in content for term in [
                "agent", "ccea", "development", "testing", "dry-run", "legacy"
            ])
            assert has_ccea_context, \
                f"{md_file} mentions script_live.py without CCEA/Agent context"
...
class TestArchiveExists:
    ...
    def test_archive_has_readme(self, archive_dir):
        ...
        assert has_readme or has_deprecated or any(archive_dir.iterdir()), \
