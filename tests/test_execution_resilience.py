...
from packages.agent.execution.resilience import (
    CircuitBreaker, CircuitOpenError, CircuitState, MaxRetriesExceeded,
    OrderStatusPoller, ResilientExecutor, RetryPolicy, StartupReconciler,
)
...
def test_breaker_opens_and_half_opens():
    ...
    for _ in range(3):
        cb.record_attempt(); cb.record_failure()
...
def test_breaker_halfopen_failure_reopens():
    ...
    clock["t"] = 11.0
    assert cb.state == CircuitState.HALF_OPEN
    cb.record_attempt(); cb.record_failure()   # провал в half-open → снова OPEN
    assert cb.state == CircuitState.OPEN
...
def test_executor_retries_then_succeeds():
    ...
    def flaky():
        ...
        return "ok"
    ex = ResilientExecutor(retry=RetryPolicy(max_attempts=5, base_delay=1.0),
...
def test_executor_raises_after_max():
    ...
    ex = ResilientExecutor(retry=RetryPolicy(max_attempts=3, base_delay=0.1),
                           sleep_fn=lambda d: None)
...
def test_executor_circuit_open_rejects():
    ...
    ex = ResilientExecutor(retry=RetryPolicy(max_attempts=10, base_delay=0.0),
                           breaker=cb, sleep_fn=lambda d: None)
    def always_fail():
...
def test_poller_classifies_terminal_and_filled():
    db = {"o1": {"status": "FILLED", "filled_qty": 100},
          "o2": {"status": "NEW"},
...
def test_reconcile_clean_when_matched():
    ...
    rep = rec.reconcile(
        local_open_order_ids=["a"], broker_open_order_ids=["a"],
        local_positions={"AAPL": 100}, broker_positions={"AAPL": 100},
    )
