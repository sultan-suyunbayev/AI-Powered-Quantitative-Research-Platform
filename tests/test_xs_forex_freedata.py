            ...
            s: pd.DataFrame(
                {
                    ...
                    "symbol": s,
                    "close": 1.1 + 0.001 * np.arange(self.n),
                }
            )
            for s in symbols
        }
...
def _panel(n=6, syms=("EURUSD", "USDJPY")):
    ...
    for s in syms:
        ...
        frames[s] = pd.DataFrame(
            {"timestamp": ts, "symbol": s, "close": 1.1 + 0.001 * np.arange(n)}
        )
...
def test_build_forex_enricher_with_and_without_rates():
    ...
    cfg2 = XSConfig.model_validate(
        {"asset_class": "forex", "data": {"source": "free", "enrich": ["rate_diff"]}}
    )
