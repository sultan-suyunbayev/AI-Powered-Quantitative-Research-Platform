...
def test_new_order_single_roundtrip_and_checksum():
    msg = new_order_single(
        cl_ord_id="ORD1", symbol="AAPL", side=Side.BUY, qty=100, ord_type=OrdType.LIMIT, price=150.0
    )
...
def test_execution_report_and_cancel():
    ...
    cxl = order_cancel_request(
        orig_cl_ord_id="ORD1", cl_ord_id="ORD2", symbol="AAPL", side=Side.BUY
    )
...
def test_best_venue_picks_lowest_cost():
    ...
    venues = [
        Venue("CHEAP", fee_bps=0.5, liquidity=1e7, impact_coef=0.1),
...
def test_best_venue_prefers_liquidity_for_impact():
    ...
    venues = [
        Venue("THIN", fee_bps=1.0, liquidity=1e6, impact_coef=0.1),
...
def test_min_notional_respected():
    venues = [
        Venue("A", fee_bps=0.5, liquidity=1e6, min_notional=500_000),
