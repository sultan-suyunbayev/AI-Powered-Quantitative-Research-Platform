...
def test_router_primary_used_when_healthy():
    r = MarketDataRouter(
        [("primary", lambda s, **k: _bars()), ("backup", lambda s, **k: _bars())],
...
def test_router_fails_over_on_error():
    ...
    r = MarketDataRouter(
        [("primary", bad), ("backup", lambda s, **k: _bars())],
        monitor=DataQualityMonitor(staleness_seconds=None),
    )
    out = r.get_bars("AAPL")
...
def test_router_fails_over_on_bad_data():
    ...
    r = MarketDataRouter(
        [
            ("primary", stale),
            ("backup", lambda s, **k: _bars(start_ms=int(time.time() * 1000) - 600000)),
        ],
...
def test_router_all_down():
    ...
    r = MarketDataRouter(
        [("a", bad), ("b", bad)], monitor=DataQualityMonitor(staleness_seconds=None)
    )
    out = r.get_bars("AAPL")
