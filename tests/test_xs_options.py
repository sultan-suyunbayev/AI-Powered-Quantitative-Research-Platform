...
from service_options_portfolio import (
    OptionLeg, GreeksNeutralConstraints, OptionsPortfolioConstructor,
    construct_options_portfolio, synthetic_option_book,
)
...
def test_too_few_legs_collapses_to_neutral_zero():
    ...
    legs = [OptionLeg("X", spot=100, strike=100, time_to_expiry=0.25, iv=0.2, is_call=True, alpha=1.0)]
    port = construct_options_portfolio(legs, neutralize=["delta"], gross_max=1.0)
...
def test_max_position_clip_diversifies():
    ...
    capped = construct_options_portfolio(legs, neutralize=["delta", "vega"], gross_max=1.0, max_position=0.05)
...
def _opt_panel(n=4):
    ...
    for sym, iv, rv in [("SPX", 0.20, 0.14), ("AAPL", 0.30, 0.28)]:
        frames[sym] = pd.DataFrame({
            "timestamp": ts, "symbol": sym, "close": 100.0,
            "iv": iv, "realized_vol": rv,
            "iv_put_25": iv + 0.03, "iv_call_25": iv - 0.01,
            "iv_front": iv + 0.02, "iv_back": iv,
        })
...
def test_build_options_exposures():
    ...
    B = build_options_exposures(ivc, skews={"SPX": 0.05, "AAPL": 0.02, "MSFT": 0.01},
