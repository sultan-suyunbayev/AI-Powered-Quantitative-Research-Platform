...
def test_mask_creation_in_rollout_buffer():
    ...
    try:
        ...
        assert (
            "mask_np = self.pad_and_flatten(np.ones_like(" in source
...
def test_advantages_are_all_valid_in_buffer():
    ...
    try:
        ...
        assert np.all(np.isfinite(advantages_flat)), "All advantages in buffer should be finite"

        assert (
            advantages_flat.size == buffer_size * n_envs
...
def test_normalization_with_extreme_values():
    ...
    for name, advantages in test_cases:
        try:
            ...
            # Verify no NaN/Inf
            assert np.all(np.isfinite(normalized)), f"{name}: normalized values should be finite"

            # Verify normalized mean ≈ 0
            norm_mean = float(np.mean(normalized))
            assert abs(norm_mean) < 1e-5, f"{name}: normalized mean should be ≈0, got {norm_mean}"
...
def test_normalization_with_constant_values():
    ...
    try:
        ...
        assert np.allclose(normalized, 0.0, atol=1e-6), "Constant advantages should normalize to 0"

        assert np.all(np.isfinite(normalized)), "Normalized constant values should be finite"
...
def test_normalization_with_single_outlier():
    ...
    try:
        ...
        assert np.isfinite(normalized[-1]), "Outlier should have finite normalized value"
...
def test_float32_vs_float64_precision():
    ...
    try:
        ...
        assert np.all(np.isfinite(normalized)), "Float32/64 conversion should produce finite values"
...
def test_single_value_buffer():
    ...
    try:
        ...
        assert np.allclose(normalized, 0.0), "Single value should normalize to 0"
...
def test_two_values_opposite_signs():
    ...
    try:
        ...
        assert abs(normalized[0] + normalized[1]) < 1e-6, "Normalized values should be symmetric"
...
def test_implementation_uses_float64_for_computation():
    ...
    try:
        ...
        assert (
            ".astype(np.float64)" in source
        ), "Should convert to float64 for statistics computation"

        # Should compute mean/std on float64
        assert "np.mean(advantages_flat)" in source, "Should use numpy mean on flattened advantages"
        ...
        assert ".astype(np.float32)" in source, "Should convert back to float32 for storage"
...
def test_implementation_has_std_clamping():
    ...
    try:
        ...
        # Should clamp std
        assert (
            "max(adv_std, 1e-8)" in source or "adv_std_clamped" in source
...
def test_implementation_checks_normalize_advantage_flag():
    ...
    try:
        ...
        assert "if self.normalize_advantage" in source, "Should check normalize_advantage flag"

        assert (
            "rollout_buffer.advantages is not None" in source
...
def test_implementation_normalizes_in_place():
    ...
    try:
        ...
        assert "rollout_buffer.advantages = " in source, "Should update buffer.advantages in-place"

        # Should NOT create a new buffer
        assert "new_buffer" not in source.lower(), "Should not create new buffer"
...
def test_implementation_logs_statistics():
    ...
    try:
        ...
        assert (
            'logger.record("train/advantages_mean_raw"' in source
        ), "Should log raw mean before normalization"

        assert (
            'logger.record("train/advantages_std_raw"' in source
...
def test_train_does_not_renormalize():
    ...
    try:
        ...
        assert "advantages.mean()" not in source, "train() should NOT compute advantage mean"

        assert "advantages.std(" not in source, "train() should NOT compute advantage std"

        # Should NOT have group normalization
        assert "group_adv_mean" not in source, "train() should NOT have group-level normalization"

        # Should use advantages directly
        assert (
            "advantages_selected = advantages_flat[valid_indices]" in source
            or "advantages_selected = advantages" in source
            or "advantages_selected = advantages_flat" in source
...
def test_normalized_distribution_properties():
    ...
    try:
        ...
        distributions = [
            ...
            ("Bimodal", np.concatenate([np.random.randn(500) - 5, np.random.randn(500) + 5])),
        ]
...
def test_advantages_remain_constant_across_epochs():
    ...
    try:
        ...
        for epoch in range(5):
            ...
            assert abs(epoch_mean) < 1e-5, f"Epoch {epoch}: mean should stay ≈0, got {epoch_mean}"

            assert (
                abs(epoch_std - 1.0) < 1e-5
...
def test_matches_stable_baselines3_normalization():
    ...
    try:
        ...
        assert np.allclose(
            our_normalized, sb3_normalized, rtol=1e-5
