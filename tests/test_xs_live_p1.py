...
class _Prices:
    ...
    def get_prices(self): return self._d


class _Adv:
    def __init__(self, d): self._d = d
    def get_adv(self): return self._d


def _cov(syms, vol=0.3, rho=0.5):
    n = len(syms); C = np.full((n, n), rho); np.fill_diagonal(C, 1.0)
    D = np.diag([vol] * n)
...
def test_pretrade_var_blocks_rebalance():
    ...
    runner = CrossSectionalLiveRunner(
        pretrade_analyzer=an, risk_limits=RiskLimits(var_max=0.01))  # очень жёсткий VaR
...
def test_pretrade_passes_and_builds_execution_plan():
    ...
    runner = CrossSectionalLiveRunner(
        pretrade_analyzer=an, risk_limits=RiskLimits(var_max=10.0),   # мягко
        scheduler=sched,
