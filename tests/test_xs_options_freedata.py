...
class FakePriceSource:
    ...
    def get_bars(self, symbols, timeframe, *, start_ms=None, end_ms=None, limit=1000):
        ...
        for s in symbols:
            ...
            out[s] = pd.DataFrame(
                {"timestamp": [T0 + i * STEP for i in range(self.n)], "symbol": s, "close": close}
            )
        return out
...
def test_yfinance_chain_pit_none():
    enr = YFinanceChainEnricher(
        provider=lambda s: pd.DataFrame(
            columns=["publish_ts", "symbol", "iv", "skew", "term_slope"]
        )
    )
...
def test_vrp_skew_term_come_alive():
    ...
    def iv_prov(symbols):
        rows = [
            {"publish_ts": pub, "symbol": s, "iv": 0.55, "skew": 0.04, "term_slope": 0.02}
            for s in symbols
        ]
        return pd.DataFrame(rows)

    res = DataAssembler(
        src, enrichers=[DeribitIVEnricher(provider=iv_prov), RealizedVolEnricher(window=10)]
    ).assemble(["BTC", "ETH"], "1d")
