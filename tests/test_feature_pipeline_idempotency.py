...
class TestFeaturePipelineIdempotency:
    ...
    def test_strict_mode_fails_on_repeated_transform_same_df(self, fitted_pipeline, simple_df):
        ...
        assert (
            result1.attrs.get("_feature_pipeline_transformed") == True
    ...
    def test_strict_mode_fails_on_repeated_transform_copy(self, fitted_pipeline, simple_df):
        ...
        assert (
            result1_copy.attrs.get("_feature_pipeline_transformed") == True
    ...
    def test_idempotent_mode_returns_unchanged(self, simple_df):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert "already-transformed" in str(
                w[0].message
            ), "Warning should mention already-transformed"
    ...
    def test_double_shift_causes_wrong_lag(self, simple_df):
        ...
        assert (
            df["close_shift2"].iloc[2] == 100.0
        ), "Second shift: row 2 should have row 0's value (lag=2)"
    ...
    def test_close_orig_bypass_allows_multiple_transforms(self, fitted_pipeline):
        ...
        assert (
            result["close"].iloc[0] == 100.0
        ), "close should not be shifted when close_orig exists"
    ...
    def test_save_load_preserves_strict_idempotency_flag(
        self, fitted_pipeline, simple_df, tmp_path
    ):
        ...
        assert (
            loaded_pipe.strict_idempotency == fitted_pipeline.strict_idempotency
...
class TestFeaturePipelineDataIntegrity:
    ...
    def test_shift_consistency_between_fit_and_transform(self):
        ...
        if "close_z" in result.columns:
            ...
            assert np.isfinite(
                result["close_z"].iloc[1:].values
            ).all(), "z-scores should be finite (no scale mismatch)"
