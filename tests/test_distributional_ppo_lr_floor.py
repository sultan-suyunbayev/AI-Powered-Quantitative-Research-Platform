...
def test_optimizer_lr_floor_dry_run_logs_chain() -> None:
    ...
    algo._scheduler_min_lr = 5e-6
    algo._kl_min_lr = 5e-6
    ...
    assert logs["train/scheduler_lr"] == pytest.approx(1e-5)
    ...
    assert logs["train/optimizer_lr_min"] == pytest.approx(5e-6)
    assert logs["train/scheduler_lr_min"] == pytest.approx(5e-6)
