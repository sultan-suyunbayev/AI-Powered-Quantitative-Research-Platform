...
class TestVGSv31Fix:
    ...
    def test_mean_of_squares_not_square_of_mean(self):
        ...
        # Create VGS
        vgs = VarianceGradientScaler(parameters=[param], enabled=True, beta=0.9, warmup_steps=0)
        ...
        assert (
            abs(actual_sq_ema - wrong_grad_sq) > 1.0
    ...
    def test_variance_underestimation_fixed_for_large_params(self):
        ...
        assert (
            abs(actual_sq_ema - expected_grad_sq_correct) < 1e-6
    ...
    def test_variance_tracking_over_time_correct(self):
        ...
        # Should match
        assert (
            abs(actual_mean_ema - expected_mean_ema) < 1e-4
        ), f"E[mean(g)] mismatch: expected {expected_mean_ema}, got {actual_mean_ema}"

        assert (
            abs(actual_sq_ema - expected_sq_ema) < 1e-3
    ...
    def test_migration_warning_from_v3_0(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert (
                "mean of squares" in warning_text or "square of mean" in warning_text
