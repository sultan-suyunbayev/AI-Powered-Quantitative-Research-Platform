...
class TestLookAheadBias:
    ...
    def test_no_future_data_in_features(self):
        ...
        assert (
            abs(actual_sma - expected_sma) < 0.01
    ...
    def test_asof_merge_no_forward_leakage(self):
        ...
        spec = AsofSpec(
            name="future", df=future_source, time_col="ts_ms", keys=["symbol"], direction="backward"
        )
...
class TestTimeAlignment:
    ...
    def test_features_bar_duration_consistency(self):
        ...
        spec = FeatureSpec(
            lookbacks_prices=[240, 480, 1440], bar_duration_minutes=240  # 4h, 8h, 24h в минутах
        )
...
class TestMissingDataHandling:
    ...
    def test_insufficient_warmup_handling(self):
        ...
        if "sma_14400" in result.columns:
            assert (
                result["sma_14400"].isna().all()
            ), "Недостаточные данные должны быть помечены как NaN"
...
class TestOutliersAndAnomalies:
    ...
    def test_zero_volume_handling(self):
        ...
        """
        spec = FeatureSpec(
            lookbacks_prices=[240], taker_buy_ratio_windows=[480], bar_duration_minutes=240
        )
        ...
        if "taker_buy_ratio" in result.columns:
            ...
            ).any(), "Нулевой объем не должен приводить к inf в taker_buy_ratio"
    ...
    def test_negative_values_graceful_handling(self):
        ...
        df = pd.DataFrame(
            {
                ...
                "symbol": ["BTCUSDT"] * 10,
                "price": [-29000.0]
                + [29000.0 + i * 100 for i in range(1, 10)],  # отрицательная первая цена
            }
        )
        ...
        assert (
            result["ref_price"].iloc[0] < 0
        ), "Feature pipeline не должен фильтровать отрицательные значения"
...
class TestFeatureLeakage:
    ...
    def test_features_independent_of_future_labels(self):
        ...
        result1 = apply_offline_features(
            df1, spec=spec, ts_col="ts_ms", symbol_col="symbol", price_col="price"
        )
        result2 = apply_offline_features(
            df2, spec=spec, ts_col="ts_ms", symbol_col="symbol", price_col="price"
        )

        # Признаки до момента 5 должны быть идентичны (будущее не влияет)
        for i in range(5):
            assert (
                abs(result1.iloc[i]["sma_240"] - result2.iloc[i]["sma_240"]) < 0.01
    ...
    def test_online_offline_feature_consistency(self):
        ...
        for _, row in df.iterrows():
            features = transformer.update(
                symbol="BTCUSDT", ts_ms=int(row["ts_ms"]), close=float(row["price"])
            )
        ...
        assert (
            abs(offline_sma - online_sma) < 0.01
