...
class FakeInner:
    ...
    def get_bars(self, symbols, timeframe, *, start_ms=None, end_ms=None, limit=1000):
        ...
        for tk in symbols:
            out[tk] = pd.DataFrame(
                {"timestamp": [T0, T0 + STEP], "symbol": tk, "close": [100.0, 101.0]}
            )
        return out
...
def test_carry_enricher_pit():
    ...
    def fb(symbols):
        return pd.DataFrame(
            {"publish_ts": [pub], "symbol": ["ES"], "front": [70.0], "back": [68.0]}
        )
...
def test_price_source_routing():
    ...
    fut = XSConfig.model_validate(
        {"asset_class": "futures", "data": {"source": "free", "vendor": "yahoo", "symbols": ["ES"]}}
    )
    assert isinstance(_price_source_for(fut), ContinuousProxySource)
    cry = XSConfig.model_validate(
        {
            "asset_class": "crypto",
