...
CEIL = LimitCeiling(max_turnover=0.10, max_notional_per_rebalance=100_000.0,
                    max_orders_per_rebalance=25)


def make_store(tmp_path: Path, clock: Clock, key: bytes = b"audit-key") -> LiveTradingAuthorizationStore:
...
def grant(store, clock, *, broker="binance", ttl=3600, token="T", expected="T", **kw):
    return store.grant(
        strategy_id="xs-rebalance", config=CFG, broker=broker,
        limit_ceiling=CEIL, ttl_sec=ttl,
        confirmation_token=token, expected_token=expected, **kw,
    )
...
def test_check_allows_within_ceiling(tmp_path):
    ...
    store = make_store(tmp_path, clock)
    grant(store, clock)
    chk = store.check(strategy_id="xs-rebalance", config=CFG, broker="binance",
                      turnover=0.08, notional=50_000.0, n_orders=10)
...
def test_no_authorization_blocks(tmp_path):
    ...
    store = make_store(tmp_path, clock)
    chk = store.check(strategy_id="xs-rebalance", config=CFG, broker="binance",
                      turnover=0.01, notional=100.0, n_orders=1)
...
def test_config_change_invalidates_authorization(tmp_path):
    ...
    chk = store.check(strategy_id="xs-rebalance", config=changed, broker="binance",
                      turnover=0.01, notional=100.0, n_orders=1)
...
def test_broker_binding(tmp_path):
    ...
    chk = store.check(strategy_id="xs-rebalance", config=CFG, broker="oanda",
                      turnover=0.01, notional=100.0, n_orders=1)
...
def test_notional_budget_exhaustion(tmp_path):
    ...
    assert store.check(strategy_id="xs-rebalance", config=CFG, broker="binance",
                       turnover=0.05, notional=100_000.0, n_orders=5).allowed
    store.consume(aid, notional=100_000.0, n_orders=5)
    # второй на $100k превысит бюджет $150k → блок
    chk = store.check(strategy_id="xs-rebalance", config=CFG, broker="binance",
                      turnover=0.05, notional=100_000.0, n_orders=5)
...
def test_rebalance_count_budget(tmp_path):
    ...
    for _ in range(2):
        assert store.check(strategy_id="xs-rebalance", config=CFG, broker="binance",
                           turnover=0.01, notional=100.0, n_orders=1).allowed
...
def test_hard_caps_clamp(tmp_path):
    clock = Clock()
    store = make_store(tmp_path, clock)
    huge = LimitCeiling(max_turnover=99.0, max_notional_per_rebalance=1e12,
                        max_orders_per_rebalance=10)
    res = store.grant(strategy_id="s", config=CFG, broker="binance",
                      limit_ceiling=huge, ttl_sec=99 * 24 * 3600,
...
def test_audit_chain_valid_and_records_events(tmp_path):
    ...
    events = [json.loads(l)["event"] for l in Path(store.audit_path).read_text(encoding="utf-8").splitlines() if l.strip()]
...
def test_state_survives_reload(tmp_path):
    ...
    chk = store2.check(strategy_id="xs-rebalance", config=CFG, broker="binance",
                       turnover=0.01, notional=100.0, n_orders=1)
