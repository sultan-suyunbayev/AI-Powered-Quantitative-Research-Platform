...
def _export_service():
    from services.gdpr.data_export import (
        GDPRExportService, InMemoryUserRepository, InMemoryStrategiesRepository,
        InMemoryBacktestsRepository, InMemoryExecutionsRepository, InMemorySettingsRepository,
    )
...
def _deletion_service():
    ...
    cats = ["account", "profile", "strategies", "backtests", "execution_logs",
            "broker_credentials", "analytics", "notifications", "sessions"]
...
def test_gdpr_export_end_to_end():
    ...
    pkg = getattr(req, "data_package", None) or getattr(req, "package", None) or getattr(req, "result", None)
...
def test_dora_concentration_end_to_end():
    ...
    risk.add_provider_dependency(provider_id="AWS-EU", provider_name="AWS",
                                 services=["compute", "storage"], provider_country="IE",
                                 transaction_volume_pct=45.0)
    risk.add_provider_dependency(provider_id="GCP-EU", provider_name="GCP",
                                 services=["compute"], provider_country="BE",
