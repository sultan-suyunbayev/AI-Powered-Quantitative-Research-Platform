...
def test_watch_seasonality_file_retains_last_good(tmp_path, caplog):
    path = tmp_path / "mult.json"
    good = {"latency": [1.0] * HOURS_IN_WEEK}
    path.write_text(json.dumps(good))

    calls = []
    # The failure is reported through the "seasonality" logger, not stderr.
    caplog.set_level(logging.WARNING, logger="seasonality")
    ...
    assert any(
        "Failed to reload seasonality multipliers" in record.getMessage()
