...
def test_sor_live_liquidity_and_dispatch():
    ...
    venues = [Venue("NYSE", fee_bps=0.5, impact_coef=0.05), Venue("NASDAQ", fee_bps=0.6, impact_coef=0.05)]
    quotes = {"NYSE": VenueQuote(99.98, 100.02, 5000, 5000),
    ...
    sent = []
    disp = sor.dispatch(route, lambda v, s, sd, n: (sent.append(v) or {"success": True, "broker_order_id": v}))
...
def test_robust_and_multiperiod():
    from service_optimizer import (PortfolioOptimizer, OptimizerConstraints, TCostModel,
                                   RobustConfig, MultiPeriodOptimizer)
    syms = [f"S{i}" for i in range(6)]
    mu = pd.Series([0.05] * 6, index=syms)
    Sigma = pd.DataFrame(np.eye(6) * 0.04, index=syms, columns=syms)
    base = PortfolioOptimizer(objective="mean_variance", risk_aversion=3.0,
                              constraints=OptimizerConstraints(gross_max=2.0), tcost=TCostModel(linear=0))
    w0 = base.solve(mu, Sigma)
    rob = PortfolioOptimizer(objective="mean_variance", risk_aversion=3.0,
                             constraints=OptimizerConstraints(gross_max=2.0), tcost=TCostModel(linear=0),
...
def test_per_name_sqrt_impact():
    ...
    opt = PortfolioOptimizer(objective="mean_variance", risk_aversion=3.0,
                             constraints=OptimizerConstraints(gross_max=1.0), tcost=tc)
...
def test_equity_barra_factors_from_fundamentals():
    ...
    B = build_equity_exposures(rw, mcaps={s: 1e9 * (i + 1) for i, s in enumerate(syms)},
                               earnings={s: 1e8 * (i + 1) for i, s in enumerate(syms)},
                               book={s: 5e8 for s in syms},
                               roe={s: 0.1 * (i + 1) for i, s in enumerate(syms)}, momentum_lookback=30)
...
def test_market_abuse_spoofing_and_wash():
    from services.algo_integration.market_abuse import (MarketAbuseMonitor, MarketAbuseConfig,
                                                        OrderEvent, TradeEvent)
    m = MarketAbuseMonitor(MarketAbuseConfig(spoof_large_qty=1000, spoof_cancel_ms=5000,
                                             spoof_min_events=3, spoof_min_distance_bps=5))
    t0 = 1700000000000
    for i in range(4):
        m.record_order(OrderEvent(t0 + i * 1000, "AAPL", "ACC1", "BUY", "NEW", 2000, 99.0, f"o{i}", mid=100.0))
        m.record_order(OrderEvent(t0 + i * 1000 + 500, "AAPL", "ACC1", "BUY", "CANCEL", 2000, 99.0, f"o{i}", mid=100.0))
...
def test_durable_alert_store_and_audit_chain(tmp_path):
    ...
    chain._conn.execute("UPDATE audit SET payload='{\"id\":9}' WHERE seq=2"); chain._conn.commit()
...
def test_gbm_alpha_without_sklearn():
    ...
    X = rng.normal(size=(300, 4)); y = X[:, 0] * 2 - X[:, 1]
...
def test_env_capture_and_dirty_promote_block(tmp_path):
    ...
    art = tmp_path / "model.json"; art.write_text("{}")
    mv = reg.register("m", run_id="r1", artifact_path=str(art), metrics={"s": 1.0},
...
def test_drift_ks_wasserstein_concept_and_closed_loop():
    ...
    rng = np.random.default_rng(0)
    a = rng.normal(0, 1, 2000); b = rng.normal(0.8, 1, 2000)
    ...
    calls = {"r": 0, "p": 0}
    res = sched.run_closed_loop({"f": {"psi": 0.4}},
                                retrain_fn=lambda d: (calls.__setitem__("r", 1) or type("A", (), {"name": "m"})()),
                                register_fn=lambda a, d: None, verify_fn=lambda a: True,
...
def test_average_price_allocation_and_settlement():
    ...
    out = ClearingEngine().process_block(
        ...
        trade_date=date(2026, 6, 15), asset_class="equity",
...
class _FakeBackend:
    def __init__(self): self.oid = 0; self.pos = {}
    def place(self, **k):
        self.oid += 1; q = k["qty"] * (1 if k["side"] == "BUY" else -1)
        self.pos[k["symbol"]] = self.pos.get(k["symbol"], 0) + q
        return {"success": True, "broker_order_id": f"B{self.oid}", "status": "filled",
                "filled_qty": k["qty"], "avg_price": 100.0}
    def cancel(self, b): return True
    def order(self, b): return {"status": "filled", "filled_qty": 1, "avg_price": 100.0}
    def positions(self): return [{"symbol": s, "qty": v, "avg_price": 100.0, "price": 100.0,
                                  "market_value": v * 100.0} for s, v in self.pos.items()]
    def account(self): return {"equity": 100000, "cash": 50000, "buying_power": 100000}
...
def test_agent_connectors(which):
    ...
    r = c.submit_order(OrderRequest(client_order_id="X1", symbol="SYM", side=OrderSide.BUY,
