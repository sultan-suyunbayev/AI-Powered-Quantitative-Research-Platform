...
def test_selected_features_filtering(tmp_path):
    ...
    cmd = [
        ".venv/bin/python", "make_features.py",
        "--in", str(in_path),
        "--out", str(out_path),
        "--price-col", "close",
        "--open-col", "open",
        ...
        "--volume-col", "volume",
        "--lookbacks", "240",
        "--selected-features", "rsi,sma_240,extra_greeks_delta,ema_240,williams_r_240,keltner_upper_240,adx_240,obv,mfi_240,plus_di_240,minus_di_240,std_returns_240,mom_240,donchian_upper_240,ao,cmf_240,pvt"
    ]
...
def test_options_occ_grouping_and_greeks(tmp_path):
    ...
    df = pd.DataFrame({
        ...
        "occ_symbol": ["AAPL241220C00150000", "AAPL241220C00160000", "AAPL241220C00150000", "AAPL241220C00160000"],
    ...
    cmd = [
        ...
        "--price-col", "mid",
        "--lookbacks", "480",
        "--selected-features", "delta,sma_480"
    ]
    res = subprocess.run(cmd, capture_output=True, text=True)
    assert res.returncode == 0, f"make_features.py failed: {res.stderr}"
    ...
    row1 = out_df[(out_df["occ_symbol"] == "AAPL241220C00150000") & (out_df["ts_ms"] == 1700000240000)].iloc[0]
    row2 = out_df[(out_df["occ_symbol"] == "AAPL241220C00160000") & (out_df["ts_ms"] == 1700000240000)].iloc[0]
