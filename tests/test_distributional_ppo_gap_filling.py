...
def test_train_sa_ppo_and_weighted_entropy():
    ...
    class _SaPpoStub:
        ...
        def compute_robust_kl_penalty(self, states_clean, states_adv, actions, **kwargs):
            # The trainer passes the recurrent state and the sample mask by
            # keyword, as the real wrapper declares them.
            assert states_clean.shape == states_adv.shape, (
                "the robust KL is between a state and its own perturbation, so "
                "the two sides have to be the same rows"
            )
