...
def test_code_structure_analysis():
    ...
    assert (
        "_build_support_distribution" not in section
    ...
    assert (
        "_project_categorical_distribution" in section
    ...
    assert "mean(max(L_unclipped, L_clipped))" in section, "Correct formula documentation missing!"
    ...
    if "critic_loss_per_sample_normalized" in section:
        ...
        if len(after_definition) > 0:
            ...
            non_comment_usage = [
                part for part in after_definition if not part.strip().startswith("#")
            ]
            if non_comment_usage and any(
                "critic_loss_per_sample_normalized" in part for part in non_comment_usage
            ):
...
def test_edge_cases_analysis():
    ...
    for test_case in test_cases:
        ...
        for i in range(len(L_u)):
            ...
            assert (
                abs(double_max[i] - expected_max) < 1e-9
            ), f"Edge case '{name}': max at index {i} incorrect"

        # 2. Mean is correct
        expected_mean = sum(double_max) / len(double_max)
        assert abs(loss - expected_mean) < 1e-9, f"Edge case '{name}': mean incorrect"
...
def test_comparison_with_quantile_vf_clipping():
    ...
    categorical_has_mean_max = (
        "torch.mean" in categorical_section and "torch.max" in categorical_section
    )
