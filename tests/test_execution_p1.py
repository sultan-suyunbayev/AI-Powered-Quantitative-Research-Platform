...
from packages.agent.execution.fix_protocol import (
    order_cancel_replace_request, parse_message, verify_checksum, MsgType, Tag, Side, OrdType,
)
from packages.agent.execution.engine import (
    LiveExecutionEngine, PriceCollarConfig, Order, OrderType, OrderStatus,
)
...
def test_fix_cancel_replace_message():
    msg = order_cancel_replace_request(
        orig_cl_ord_id="O1", cl_ord_id="O2", symbol="AAPL", side=Side.BUY,
        qty=100, ord_type=OrdType.LIMIT, price=150.25)
...
def test_is_algo_in_build_plan():
    ...
    plan = sch.build_plan(
        pd.Series({"AAPL": 0.5}), pd.Series({"AAPL": 0.0}),
        pd.Series({"AAPL": 100.0}), equity=100_000)
    assert plan.algo == "IS"
...
def _engine(collar, broker_submit=None, broker_cancel=None, broker_replace=None):
    ...
    return LiveExecutionEngine(
        ...
        broker_cancel=broker_cancel, broker_replace=broker_replace,
        order_journal=j, price_collar=collar, broker_name="sim")


def _intent(symbol="AAPL", qty="100", limit=None):
    return OrderIntent(
        strategy_id="s", symbol=symbol, intent_type=IntentType.LIMIT_ENTRY if limit else IntentType.MARKET_ENTRY,
        side=IntentSide.LONG, target_quantity=Decimal(qty),
...
def test_engine_cancel_order():
    cancelled = {}
    eng = _engine(PriceCollarConfig(enabled=False),
...
def test_engine_replace_order():
    eng = _engine(PriceCollarConfig(max_price_distance_pct=0.5),
                  broker_replace=lambda coid, q, p: (True, "B2", None))
    res = eng.execute(_intent(qty="100", limit=100))
    coid = res.order.client_order_id
    rr = eng.replace_order(coid, new_quantity=Decimal("150"), new_limit_price=Decimal("101"),
