...
def test_sector_cap_binds_scipy():
    ...
    w_no = PortfolioOptimizer(objective="mean_variance", risk_aversion=0.5, constraints=base).solve(
        mu, Sigma
    )
    assert sum(abs(w_no[s]) for s in syms[:5]) > 0.30  # unconstrained piles into tech

    capd = OptimizerConstraints(
        gross_max=2.0, max_position=1.0, sector_map=sm, sector_caps={"tech": 0.30}
    )
    w = PortfolioOptimizer(objective="mean_variance", risk_aversion=0.5, constraints=capd).solve(
        mu, Sigma
    )
...
def test_auto_reconciliation_halt_and_flatten_and_heal():
    ...
    sync = PositionSynchronizer(
        position_provider=P(),
        local_state_getter=lambda: dict(local),
        config=SyncConfig(
            consecutive_halt_threshold=2, halt_on_unreconciled=True, auto_flatten=True
        ),
...
def test_no_halt_without_persistence():
    ...
    sync = PositionSynchronizer(
        position_provider=P(),
        local_state_getter=lambda: {"AAPL": Decimal("100")},
