...
def sup():
    ...
    s = CCEASupervisor(SupervisorConfig(
        data_dir=Path(tempfile.mkdtemp(prefix="ccea_order_test_")), paper=True))
    s.start()
...
def test_limit_order_carries_limit_price(sup):
    r = sup.submit_manual_order(symbol="BTCUSDT", side="buy", order_type="limit",
                                quantity=0.1, limit_price=49_000.0, time_in_force="GTC")
    assert r["ok"] is True and r["order_type"] == "limit"
    assert r["limit_price"] == 49_000.0


def test_stop_order_carries_stop_price(sup):
    r = sup.submit_manual_order(symbol="BTCUSDT", side="buy", order_type="stop",
                                quantity=0.1, stop_price=51_000.0)
    assert r["ok"] is True and r["order_type"] == "stop" and r["stop_price"] == 51_000.0


def test_stop_limit_requires_both_prices(sup):
    assert sup.submit_manual_order(symbol="BTCUSDT", side="buy", order_type="stop_limit",
                                   quantity=0.1, stop_price=51_000.0)["ok"] is False  # нет limit
    r = sup.submit_manual_order(symbol="BTCUSDT", side="buy", order_type="stop_limit",
                                quantity=0.1, stop_price=51_000.0, limit_price=51_100.0)
    assert r["ok"] is True
...
@pytest.mark.parametrize("kw,err", [
    ...
    (dict(symbol="BTCUSDT", side="buy", order_type="market", quantity=0.1, time_in_force="XXX"), "time_in_force"),
])
...
def test_reduce_only_without_position_rejected(sup):
    r = sup.submit_manual_order(symbol="BTCUSDT", side="sell", order_type="market",
                                quantity=0.1, reduce_only=True)
...
def test_reduce_only_wrong_side_rejected(sup):
    ...
    r = sup.submit_manual_order(symbol="BTCUSDT", side="buy", order_type="market",
                                quantity=0.1, reduce_only=True)
...
def test_api_order_submit_requires_confirm(monkeypatch, sup):
    ...
    res = client.post("/api/ccea/order/submit", json={
...
def test_api_order_submit_and_open_orders(monkeypatch, sup):
    ...
    res = client.post("/api/ccea/order/submit", json={
        ...
        "quantity": 0.5, "limit_price": 1990.0, "confirm": True})
...
def test_api_order_validation_400(monkeypatch, sup):
    ...
    res = client.post("/api/ccea/order/submit", json={
        ...
        "quantity": 0.1, "confirm": True})   # limit без цены
...
def test_api_503_without_ccea(monkeypatch):
    ...
    import services.ops_kill_switch as oks
    monkeypatch.setattr(oks, "tripped", lambda: False)
    assert client.post("/api/ccea/order/submit", json={
        "symbol": "BTCUSDT", "side": "buy", "order_type": "market",
        "quantity": 0.1, "confirm": True}).status_code == 503
