...
def test_plan_turnover_cap_scales_proportionally():
    plan = plan_rebalance(
        ...
        limits=RebalanceLimits(
            max_turnover=0.2, min_trade_notional=1.0, drift_band_bps=1.0, max_position_weight=0.5
        ),
    )
...
def test_plan_zero_crossing_split_into_two_legs():
    plan = plan_rebalance(
        ...
        equity=10_000.0,
        limits=RebalanceLimits(
            max_turnover=2.0, min_trade_notional=1.0, drift_band_bps=1.0, max_position_weight=0.5
        ),
    )
...
def test_plan_max_orders_cap_journals_dropped():
    plan = plan_rebalance(
        ...
        equity=10_000.0,
        limits=RebalanceLimits(
            max_orders=3, min_trade_notional=1.0, drift_band_bps=1.0, max_turnover=5.0
        ),
    )
...
def xs_env(tmp_path, monkeypatch):
    ...
    def fake_load_panel(cfg_obj):
        idx = pd.MultiIndex.from_product(
            [[1_700_000_000_000, 1_700_000_060_000], ["BTC", "ETH"]], names=["ts_ms", "symbol"]
        )
...
def test_runner_dry_run_sends_nothing(xs_env, tmp_path):
    sup = FakeSupervisor()
    rec = run_rebalance(
        str(xs_env), sup, dry_run=True, limits=LIM, out_dir=str(tmp_path / "journal")
    )
...
def test_runner_paper_only_blocks_live_broker(xs_env, tmp_path):
    ...
    rec = run_rebalance(str(xs_env), sup, paper_only=True, limits=LIM, out_dir=str(tmp_path / "j"))
...
def test_live_broker_without_authorization_is_fail_closed(xs_env, tmp_path):
    ...
    sup = FakeSupervisor(
        simulated=False, broker="binance", live_auth_store=store, equity=1_000_000.0
    )
    rec = run_rebalance(
        str(xs_env),
        sup,
...
def test_live_broker_with_authorization_executes_and_consumes(xs_env, tmp_path):
    ...
    g = store.grant(
        ...
        limit_ceiling=LimitCeiling(
            max_turnover=1.0, max_notional_per_rebalance=1e9, max_orders_per_rebalance=50
        ),
        ttl_sec=3600,
...
def test_live_ceiling_clamps_runtime_limits(xs_env, tmp_path):
    ...
    store.grant(
        ...
        broker="binance",
        limit_ceiling=LimitCeiling(
            max_turnover=0.05, max_notional_per_rebalance=1e9, max_orders_per_rebalance=50  # жёстче
        ),
        ttl_sec=3600,
        confirmation_token="T",
        expected_token="T",
    ...
    rec = run_rebalance(
        ...
        paper_only=False,
        limits=RebalanceLimits(
            max_turnover=0.5, min_trade_notional=1.0, drift_band_bps=1.0  # мягче — должно ужаться
        ),
        strategy_id="xs-rebalance",
        out_dir=str(tmp_path / "j"),
    )
...
def test_api_run_requires_confirmation_for_real_send(monkeypatch):
    ...
    res = client.post("/api/xs/rebalance/run", json={"config": "configs/xs.yaml", "dry_run": False})
...
class _CeremonySupervisor:
    ...
    def grant_live_trading(self, **kw):
        ...
        return self._store.grant(
            ...
            ttl_sec=kw["ttl_sec"],
            confirmation_token=kw["confirmation_token"],
            expected_token=kw["expected_token"],
            max_total_notional=kw.get("max_total_notional"),
            max_rebalances=kw.get("max_rebalances"),
            note=kw.get("note", ""),
        )

    def revoke_live_trading(self, auth_id=None, reason="operator revoke"):
        return (
            self._store.revoke(auth_id, reason=reason)
            if auth_id
            else self._store.revoke_all(reason=reason)
        )
...
def test_ceremony_request_then_grant(tmp_path, monkeypatch):
    ...
    # неверный токен → 409
    bad = client.post(
        "/api/ccea/live_trading/grant",
        json={"request_id": body["request_id"], "confirmation_token": "nope"},
    )
    ...
    ok = client.post(
        "/api/ccea/live_trading/grant",
        json={"request_id": body["request_id"], "confirmation_token": body["confirmation_token"]},
    )
...
def test_ceremony_request_rejects_sim_paper_and_missing_config(tmp_path, monkeypatch):
    ...
    assert (
        client.post(
            "/api/ccea/live_trading/request", json={"config": str(cfg), "broker": "sim_paper"}
        ).status_code
        == 400
    )
    assert (
        client.post(
            "/api/ccea/live_trading/request",
            json={"config": str(tmp_path / "nope.yaml"), "broker": "binance"},
        ).status_code
        == 404
    )
...
def test_ceremony_revoke_and_status(tmp_path, monkeypatch):
    ...
    cfg.write_text("name: x\n", encoding="utf-8")
    req = client.post(
        "/api/ccea/live_trading/request", json={"config": str(cfg), "broker": "binance"}
    ).json()
    client.post(
        "/api/ccea/live_trading/grant",
        json={"request_id": req["request_id"], "confirmation_token": req["confirmation_token"]},
    )
...
def test_ceremony_503_without_ccea(monkeypatch):
    ...
    assert (
        client.post(
            "/api/ccea/live_trading/request", json={"config": "x", "broker": "binance"}
        ).status_code
        == 503
    )
...
def test_scheduler_action_maps_statuses(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(app_module, "_CCEA_STATE", "running", raising=False)
    monkeypatch.setattr(
        sxr, "run_rebalance", lambda *a, **k: {"status": "ok", "reason": "исполнено 2/2"}
    )
