...
class DataCfg(BaseModel):
    ...
    enrich: List[str] = Field(
        default_factory=list
    )  # имена обогатителей (D0: 'mcap'; D1-D5: funding/...)
...
class UniverseCfg(BaseModel):
    ...
    membership_path: Optional[str] = (
        None  # PIT changes-файл (date,ticker,action) → survivorship-free
    )
...
class OptimizerCfg(BaseModel):
    ...
    robust: Optional[Dict[str, Any]] = (
        None  # {enabled, kind: box|ellipsoidal, kappa, mu_uncertainty?}
    )
    bl_views: Optional[Dict[str, Any]] = None  # {P, Q, omega?, tau?} — Black-Litterman views
    multi_period: Optional[Dict[str, Any]] = (
        None  # {enabled, trade_rate?, trade_cost?} — Gârleanu–Pedersen
    )
...
class RLCfg(BaseModel):
    ...
    checkpoint: Optional[str] = (
        None  # путь к артефакту (BYO/CCEA-signed); None → нейтральный сигнал
    )
...
class XSConfig(BaseModel):
    ...
    fundamentals_path: Optional[str] = (
        None  # BYO PIT parquet (publish_ts) → pit=true; иначе free снимок → none
    )
    fundamentals_fields: Optional[List[str]] = (
        None  # колонки фундаментала (earnings/book_value/fcf/roe)
    )
    ...
    policy_rates: Optional[Dict[str, float]] = (
        None  # {currency: rate} (snapshot approx; BYO history → PIT)
    )
        ...
        if u.membership_path:
            ...
            try:
                ...
                if "demo" in mp.lower():
                    _logging.getLogger(__name__).warning(
                        ...
                        "results are survivorship-biased. Supply a full membership history.",
                        mp,
                    )
            ...
            try:
                ...
                return build_index_membership_universe(
                    u.membership_path,
                    index=u.index or "CUSTOM",
                    name=f"index:{u.index or 'custom'}",
                )
            except Exception as exc:  # без файла/при ошибке — honest деградация
                _logging.getLogger(__name__).warning(
                    "index_membership load failed (%s) — fallback to StaticUniverse (survivorship-biased)",
                    exc,
                )
        else:
            _logging.getLogger(__name__).warning(
...
def _build_futures(s: "SignalCfg"):
    ...
    if s.kind == "trend":
        kw = dict(
            lookback=s.lookback or 100,
            price_col=s.price_col,
            vol_normalize=s.vol_normalize,
            vol_window=s.vol_window,
        )
    elif s.kind == "carry":
        kw = dict(
            carry_col=s.carry_col,
            roll_yield_col=s.roll_yield_col,
            front_col=s.front_col,
            back_col=s.back_col,
        )
...
def _build_forex(s: "SignalCfg"):
    ...
    if s.kind == "fx_carry":
        kw = dict(
            rate_diff_col=s.rate_diff_col,
            carry_col=s.carry_col,
            rate_base_col=s.rate_base_col,
            rate_quote_col=s.rate_quote_col,
        )
...
def _build_common(s: "SignalCfg"):
    ...
    if s.kind == "residual_momentum":
        kw = dict(
            lookback=s.lookback or 252,
            skip=s.skip or 21,
            beta_window=s.vol_window or 60,
            price_col=s.price_col,
        )
...
def _build_rl(s: "SignalCfg", cfg: "XSConfig"):
    ...
    adapter = RLInferenceAdapter(
        checkpoint=rl.checkpoint,
        utility=rl.utility,
        cvar_alpha=rl.cvar_alpha,
...
def build_crypto_factor_risk(cfg: XSConfig, panel: Panel):
    ...
    method = (
        cfg.risk.method if cfg.risk.method in ("ledoit_wolf", "sample", "ewma") else "ledoit_wolf"
    )
...
def build_equity_factor_risk(cfg: XSConfig, panel: Panel):
    ...
    mcaps = (
        cfg.mcaps
        or _latest_cross_section(panel, "market_cap")
        or _latest_cross_section(panel, "mcap")
    )
    earnings = _latest_cross_section(panel, "earnings") or _latest_cross_section(
        panel, "net_income"
    )
...
def build_optimizer(cfg: XSConfig):
    ...
    if o.beta_neutral:
        ...
        if exposures_df is None or o.beta_factor not in (
            exposures_df.columns if exposures_df is not None else []
        ):
            syms = cfg.universe.symbols or cfg.data.symbols or []
            if syms:
                col = pd.Series(1.0, index=[str(s) for s in syms], name=o.beta_factor)
                exposures_df = (
                    col.to_frame() if exposures_df is None else exposures_df.join(col, how="outer")
                )

    cons = OptimizerConstraints(
        gross_max=o.gross_max,
        net_target=o.net_target,
        long_only=o.long_only,
        max_position=o.max_position,
        max_turnover=o.max_turnover,
        sector_map=cfg.sectors,
        sector_caps=(dict(o.sector_caps) if o.sector_caps else None),
        exposures=exposures_df,
        factor_caps=factor_caps,
    )
    tcost = (
        TCostModel(linear=o.tcost_linear, quad=o.tcost_quad, coef=o.tcost_coef)
        if o.tcost_aware
        else None
    )
    sizing = (
    ...
    if o.robust and bool(o.robust.get("enabled")):
        ...
        robust = RobustConfig(
            enabled=True,
            kind=str(o.robust.get("kind", "box")),
    ...
    if o.bl_views and o.bl_views.get("P") is not None and o.bl_views.get("Q") is not None:
        bl_views = {
            ...
            "omega": (
                _np.asarray(o.bl_views["omega"], dtype="float64")
                if o.bl_views.get("omega") is not None
                else None
            ),
...
def build_backtest_config(cfg: XSConfig) -> XSBacktestConfig:
    ...
    return XSBacktestConfig(
        ...
        cov_lookback=b.cov_lookback,
        min_cov_obs=b.min_cov_obs,
        alpha_refit_every=b.alpha_refit_every,
        cost_bps=b.cost_bps,
        price_col=b.price_col,
        periods_per_year=b.periods_per_year,
    )
...
def load_panel(cfg: XSConfig) -> Panel:
    ...
    if d.source == "synthetic":
        ...
        if cfg.asset_class == "futures":
            ...
            frames = synthetic_continuous_frames(
                syms, n_bars=d.synthetic_bars, seed=d.synthetic_seed
            )
            return PanelBuilder.from_frames(frames)
...
def build_enrichers(cfg: XSConfig) -> List[Any]:
    ...
    for name in cfg.data.enrich:
        ...
        if enr is not None:
            ...
            logging.getLogger(__name__).warning(
                "enricher %r неизвестен/без данных на этой стадии — пропуск (добавится в D4-D5)",
                name,
            )
    return out
...
def assemble_free(cfg: XSConfig):
    ...
    assembler = DataAssembler(
        src, enrichers=build_enrichers(cfg), cache=cache, cache_ttl_ms=d.cache_ttl_ms
    )
...
def _provenance_for(cfg: XSConfig, panel: Panel):
    ...
    return [
        ColumnProvenance(c, "byo:parquet", "byo", PIT_TRUE, False, "BYO price files.")
        for c in panel.columns
    ]
...
def data_quality_for_config(cfg: XSConfig):
    ...
    return build_quality_report(
        panel, prov, price_col=cfg.backtest.price_col, survivorship_biased=None
    )
...
def run_backtest(cfg: XSConfig, panel: Optional[Panel] = None) -> Dict[str, Any]:
    ...
    try:
        if (
            getattr(cfg, "capacity", None) is not None
            and cfg.capacity.enabled
            and res.weights.shape[0] >= 2
        ):
            from impl_capacity import capacity_from_result

            capacity = capacity_from_result(
                res,
                adv_usd=cfg.capacity.adv_usd,
    ...
    try:
        ...
        dt = data_trust_report(
            panel, provenance, signal_library=lib, price_col=cfg.backtest.price_col
        )
        data_trust = {
            "trust_verdict": dt["trust_verdict"],
            "pit_violations": dt["pit_violations"],
            "used_worst_pit": dt["used_worst_pit"],
            "signal_lineage": dt["signal_lineage"],
        }
    ...
    return {
        ...
        "warning": (
            "SYNTHETIC DATA — results are NOT a real edge; demo only." if is_synthetic else None
        ),
    }
...
__all__ = [
    ...
    "build_alpha",
    "build_risk_model",
    "build_crypto_factor_risk",
    "build_equity_factor_risk",
    "build_futures_factor_risk",
    ...
    "build_enrichers",
    "assemble_free",
    "data_quality_for_config",
    "data_trust_for_config",
]
