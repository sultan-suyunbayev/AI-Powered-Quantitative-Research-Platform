# Advanced Features Architecture - AI-Powered Quantitative Research Platform

**Last Updated**: 2025-11-21
**Version**: 2.1

> Detailed architecture diagrams for advanced features: UPGD, VGS, Twin Critics, PBT, and Critical Fixes

---

## 🆕 UPGD Optimizer Flow

```
┌────────────────────────────────────────────────────────────────┐
│                    AdaptiveUPGD Optimizer                      │
│                  (Continual Learning)                          │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│   Input: Gradients g_t                                        │
│      │                                                         │
│      ▼                                                         │
│   ┌─────────────────────────┐                                │
│   │ 1. Compute Utility      │                                │
│   │    u_t = -g_t · p       │  (negative dot product)        │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 2. EMA Utility          │                                │
│   │    U_t = β·U_{t-1}      │                                │
│   │         + (1-β)·u_t     │  (β=0.999 default)             │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 3. Gaussian Noise       │                                │
│   │    ε ~ N(0, σ²I)        │  (σ=0.001 with VGS)            │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 4. Perturbed Gradient   │                                │
│   │    g̃_t = g_t + ε       │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 5. Utility Protection   │                                │
│   │    IF U_t < 0:          │                                │
│   │      g̃_t = -g̃_t       │  (flip direction)              │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 6. Adaptive Moments     │  (AdaptiveUPGD only)           │
│   │    m_t = β₁·m_{t-1}     │                                │
│   │         + (1-β₁)·g̃_t    │                                │
│   │    v_t = β₂·v_{t-1}     │                                │
│   │         + (1-β₂)·g̃_t²   │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 7. Parameter Update     │                                │
│   │    p_{t+1} = p_t        │                                │
│   │         - α · m̂_t/√v̂_t │  (Adam-style)                  │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│           Output: Updated parameters                          │
│                                                                │
│   Key Properties:                                             │
│   • Prevents catastrophic forgetting (utility protection)    │
│   • Maintains plasticity (noise injection)                   │
│   • Adaptive learning rates (AdaptiveUPGD)                   │
│   • Compatible with VGS (reduce σ to 0.001)                  │
│                                                                │
└────────────────────────────────────────────────────────────────┘
```

**Implementation**: [optimizers/upgd_optimizer.py](optimizers/upgd_optimizer.py)

**Documentation**: [docs/UPGD_INTEGRATION.md](docs/UPGD_INTEGRATION.md)

**Tests**: `tests/test_upgd*.py`

---

## 🆕 VGS (Variance Gradient Scaler) Flow

```
┌────────────────────────────────────────────────────────────────┐
│              Variance Gradient Scaler (VGS)                    │
│           Automatic Per-Layer Gradient Scaling                 │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│   Training Loop:                                              │
│                                                                │
│   ┌─────────────────────────┐                                │
│   │ 1. Accumulation Phase   │                                │
│   │    (N backward passes)  │  N=4 (default)                 │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ For each layer:         │                                │
│   │   gradients[i].append(  │                                │
│   │     layer.grad.clone()  │                                │
│   │   )                     │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 2. Compute Variance     │                                │
│   │    σ_i = std(grads[i])  │  Per-layer std                 │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 3. Compute Scale        │                                │
│   │    s_i = 1/(σ_i + ε)    │  ε=1e-6 for stability          │
│   │    s_i = clip(s_i,      │  clip to [1/thresh, thresh]    │
│   │              max=10.0)  │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 4. Apply Scaling        │                                │
│   │    layer.grad *= s_i    │  Scale gradients               │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 5. Optimizer Step       │                                │
│   │    optimizer.step()     │  (AdaptiveUPGD or other)       │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 6. Clear Accumulator    │                                │
│   │    gradients.clear()    │                                │
│   └─────────────────────────┘                                │
│                                                                │
│   Integration with UPGD:                                      │
│   • VGS scales gradients BEFORE UPGD noise injection         │
│   • Reduce UPGD σ to 0.001 to prevent amplification          │
│   • State dict compatibility for PBT checkpointing           │
│                                                                │
└────────────────────────────────────────────────────────────────┘
```

**Implementation**: [variance_gradient_scaler.py](variance_gradient_scaler.py)

**Integration**: [distributional_ppo.py](distributional_ppo.py) (VGS integration)

**Tests**: `tests/test_upgd_vgs*.py`

---

## 🆕 Twin Critics Architecture

```
┌────────────────────────────────────────────────────────────────┐
│                    Twin Critics Architecture                   │
│             (Reduce Overestimation Bias in Value)              │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│   Input: Observation (56D)                                    │
│      │                                                         │
│      ▼                                                         │
│   ┌─────────────────────────┐                                │
│   │  Shared Feature Encoder │                                │
│   │  (LSTM + MLP)           │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│         ┌────┴─────┐                                          │
│         ▼          ▼                                          │
│   ┌──────────┐  ┌──────────┐                                │
│   │ Critic 1 │  │ Critic 2 │  (Independent heads)            │
│   │ Head     │  │ Head     │                                │
│   └────┬─────┘  └─────┬────┘                                │
│        │              │                                       │
│        ▼              ▼                                       │
│   ┌──────────┐  ┌──────────┐                                │
│   │ Value 1  │  │ Value 2  │  (Distributional: 21-51 atoms) │
│   │ (Q-dist) │  │ (Q-dist) │                                │
│   └────┬─────┘  └─────┬────┘                                │
│        │              │                                       │
│        └──────┬───────┘                                      │
│               ▼                                               │
│   ┌─────────────────────────┐                                │
│   │ Min Operator            │                                │
│   │ V_target = min(V1, V2)  │  (Reduce overestimation)       │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   Target Value for Policy Update                             │
│                                                                │
│   Training:                                                   │
│   • Both critics updated with same target                    │
│   • Quantile regression loss (Huber)                         │
│   • CVaR regularization (worst 5% tail)                      │
│                                                                │
│   Benefits:                                                   │
│   • Reduced overestimation bias                              │
│   • Improved stability in stochastic environments            │
│   • Better convergence for distributional RL                 │
│                                                                │
└────────────────────────────────────────────────────────────────┘
```

**Implementation**: [distributional_ppo.py](distributional_ppo.py) (Twin Critics integration)

**Documentation**: [docs/twin_critics.md](docs/twin_critics.md)

**Tests**: `tests/test_twin_critics*.py`

**Research**:
- Fujimoto et al. (2018) "Addressing Function Approximation Error in Actor-Critic Methods"
- PDPPO (2025) - recent distributional PPO with twin critics

---

## 🆕 PBT (Population-Based Training) Flow

```
┌────────────────────────────────────────────────────────────────┐
│           Population-Based Training (PBT) Flow                 │
│          Evolutionary Hyperparameter Optimization              │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│   Initialize Population (N=8 agents):                         │
│                                                                │
│   ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ │
│   │ A1  │ │ A2  │ │ A3  │ │ A4  │ │ A5  │ │ A6  │ │ A7  │ │
│   │lr1  │ │lr2  │ │lr3  │ │lr4  │ │lr5  │ │lr6  │ │lr7  │ │
│   └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘ │
│      │       │       │       │       │       │       │      │
│      └───────┴───────┴───────┴───────┴───────┴───────┘      │
│                            │                                  │
│   ┌────────────────────────▼────────────────────────┐       │
│   │  Training Phase (10 steps)                      │       │
│   │  Each agent trains independently                │       │
│   └────────────────────────┬────────────────────────┘       │
│                            │                                  │
│                            ▼                                  │
│   ┌─────────────────────────────────────────────────┐       │
│   │  Evaluation Phase                               │       │
│   │  Measure performance (mean_reward)              │       │
│   └────────────────────────┬────────────────────────┘       │
│                            │                                  │
│                            ▼                                  │
│   ┌─────────────────────────────────────────────────┐       │
│   │  Exploit Phase                                  │       │
│   │  Sort by performance: A7 > A3 > A1 > ... > A5  │       │
│   │  Truncation selection (top 25% vs bottom 25%)   │       │
│   │                                                 │       │
│   │  Bottom agents copy weights from top agents:   │       │
│   │    A5.weights ← A7.weights                     │       │
│   │    A2.weights ← A3.weights                     │       │
│   └────────────────────────┬────────────────────────┘       │
│                            │                                  │
│                            ▼                                  │
│   ┌─────────────────────────────────────────────────┐       │
│   │  Explore Phase                                  │       │
│   │  Perturb hyperparameters (×1.2 or ÷1.2):       │       │
│   │    lr_new = lr_old × 1.2  (or ÷1.2)            │       │
│   │                                                 │       │
│   │  OR Resample from uniform distribution:        │       │
│   │    lr_new ~ Uniform[lr_min, lr_max]            │       │
│   └────────────────────────┬────────────────────────┘       │
│                            │                                  │
│                            ▼                                  │
│   Repeat (Training → Eval → Exploit → Explore)              │
│                                                                │
│   Hyperparameters optimized:                                 │
│   • learning_rate                                            │
│   • adversarial_epsilon (for SA-PPO)                         │
│   • clip_range                                               │
│   • ent_coef                                                 │
│                                                                │
└────────────────────────────────────────────────────────────────┘
```

**Implementation**: [adversarial/pbt_scheduler.py](adversarial/pbt_scheduler.py)

**Integration**: [training_pbt_adversarial_integration.py](training_pbt_adversarial_integration.py)

**Config**: [configs/config_pbt_adversarial.yaml](configs/config_pbt_adversarial.yaml)

**Tests**: `tests/test_pbt*.py`

**Research**: Jaderberg et al. (2017) "Population Based Training of Neural Networks"

---

## 🔴 Critical Fixes Architecture (2025-11-21)

### LSTM Episode Boundary Reset

```
┌────────────────────────────────────────────────────────────────┐
│          LSTM State Management (CRITICAL FIX)                  │
│        Prevent Temporal Leakage Between Episodes               │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│   Rollout Loop:                                               │
│                                                                │
│   ┌─────────────────────────┐                                │
│   │ 1. Collect Experience   │                                │
│   │    obs, actions, ...    │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 2. Environment Step     │                                │
│   │    obs', reward, done   │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 3. Check done flags     │                                │
│   │    IF any done == True  │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│         YES  │  NO                                            │
│         ┌────┴────┐                                           │
│         ▼         ▼                                           │
│   ┌──────────┐  Continue                                     │
│   │ 4. RESET │                                               │
│   │ LSTM     │  ⚠️ NEW (2025-11-21)                          │
│   │ states   │                                               │
│   │          │  Method:                                      │
│   │ self._last_lstm_states =                                │
│   │   _reset_lstm_states_for_done_envs(                     │
│   │     lstm_states=self._last_lstm_states,                 │
│   │     episode_starts=episode_starts                       │
│   │   )                                                      │
│   └──────────┘                                               │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ 5. Continue Rollout     │                                │
│   │    Clean LSTM state     │                                │
│   │    No temporal leakage  │                                │
│   └─────────────────────────┘                                │
│                                                                │
│   Impact:                                                     │
│   • Prevents information flow between unrelated episodes     │
│   • Improves value estimation accuracy (5-15%)               │
│   • Critical for recurrent policies                          │
│                                                                │
│   Location: distributional_ppo.py:7418-7427                  │
│   Tests: tests/test_lstm_episode_boundary_reset.py (8 tests) │
│                                                                │
└────────────────────────────────────────────────────────────────┘
```

**Documentation**: [CRITICAL_LSTM_RESET_FIX_REPORT.md](CRITICAL_LSTM_RESET_FIX_REPORT.md)

**Tests**: [tests/test_lstm_episode_boundary_reset.py](tests/test_lstm_episode_boundary_reset.py)

**Research**: Hausknecht & Stone (2015) "Deep Recurrent Q-Learning for Partially Observable MDPs"

---

### Action Space Semantics Fix

```
┌────────────────────────────────────────────────────────────────┐
│           Action Space Semantics (CRITICAL FIX)                │
│          DELTA → TARGET Position Semantics                     │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│   OLD (DELTA - WRONG):                                        │
│                                                                │
│   ┌─────────────────────────┐                                │
│   │ Current position: 1000  │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ Action: +0.5            │  (increase by 50%)             │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ ❌ DELTA Interpretation │                                │
│   │ new = cur + (0.5*max)   │                                │
│   │ new = 1000 + 500        │                                │
│   │ new = 1500              │  Position DOUBLED!             │
│   └─────────────────────────┘                                │
│                                                                │
│   NEW (TARGET - CORRECT):                                     │
│                                                                │
│   ┌─────────────────────────┐                                │
│   │ Current position: 1000  │                                │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ Action: +0.5            │  (target 50% of max)           │
│   └──────────┬──────────────┘                                │
│              │                                                 │
│              ▼                                                 │
│   ┌─────────────────────────┐                                │
│   │ ✅ TARGET Interpretation│                                │
│   │ new = 0.5 * max         │                                │
│   │ new = 500               │  Correct target position       │
│   └─────────────────────────┘                                │
│                                                                │
│   Impact:                                                     │
│   • Prevents position doubling bug                           │
│   • Prevents 2x leverage violation in production             │
│   • Models with DELTA semantics MUST retrain                 │
│                                                                │
└────────────────────────────────────────────────────────────────┘
```

**Documentation**: [CRITICAL_FIXES_COMPLETE_REPORT.md](CRITICAL_FIXES_COMPLETE_REPORT.md)

**Tests**: [tests/test_critical_action_space_fixes.py](tests/test_critical_action_space_fixes.py)

---

## References

### Academic Papers
- **UPGD**: Farahmand et al. (2017) "Utility-based Perturbed Gradient Descent"
- **Twin Critics**: Fujimoto et al. (2018) "Addressing Function Approximation Error in Actor-Critic Methods"
- **PBT**: Jaderberg et al. (2017) "Population Based Training of Neural Networks"
- **LSTM in RL**: Hausknecht & Stone (2015) "Deep Recurrent Q-Learning for Partially Observable MDPs"
- **Distributional RL**: Dabney et al. (2018) "Distributional Reinforcement Learning with Quantile Regression"

### Project Documentation
- [docs/UPGD_INTEGRATION.md](docs/UPGD_INTEGRATION.md)
- [docs/twin_critics.md](docs/twin_critics.md)
- [CRITICAL_LSTM_RESET_FIX_REPORT.md](CRITICAL_LSTM_RESET_FIX_REPORT.md)
- [CRITICAL_FIXES_COMPLETE_REPORT.md](CRITICAL_FIXES_COMPLETE_REPORT.md)
- [NUMERICAL_ISSUES_FIX_SUMMARY.md](NUMERICAL_ISSUES_FIX_SUMMARY.md)
- [REGRESSION_PREVENTION_CHECKLIST.md](REGRESSION_PREVENTION_CHECKLIST.md)

---

**Maintained by**: Development Team + Claude Code
**Last Updated**: 2025-11-21
**Version**: 2.1
