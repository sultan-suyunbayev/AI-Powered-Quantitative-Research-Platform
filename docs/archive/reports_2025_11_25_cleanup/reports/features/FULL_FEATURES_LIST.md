...
**ТЕКУЩАЯ КОНФИГУРАЦИЯ (ноябрь 2025):**
- N_FEATURES = **56**
- EXT_NORM_DIM = **21** (расширено с 8 до 21)
- MAX_NUM_TOKENS = **1**
- observation_space.shape = **(56,)**

Для актуальной структуры см. `OBSERVATION_MAPPING.md` и `FEATURE_MAPPING_56.md`.

---

## ИСТОРИЧЕСКАЯ ИНФОРМАЦИЯ: Откуда взялось число 57?
...
### После исправления (январь 2025):
✅ **Всё стало 43 -- идеальное соответствие между объявлением и реализацией!**

### ТЕКУЩЕЕ СОСТОЯНИЕ (ноябрь 2025):
✅ **N_FEATURES = 56 -- с расширенным EXT_NORM_DIM=21 для дополнительных технических индикаторов**

**Изменения с января 2025:**
- EXT_NORM_DIM: 8 → **21** (добавлены производные taker_buy_ratio и другие индикаторы)
- Метаданные: 2 → **5** (добавлены risk_off_flag, fear_greed_value, fear_greed_indicator)
- observation_space.shape: (43,) → **(56,)**
- **13 новых признаков** включены в расширенный блок external

---

## 🔍 Реализованные улучшения (ноябрь 2025)

Следующие улучшения РЕАЛИЗОВАНЫ:

1. ✅ **Добавлены taker_buy_ratio** признаки в norm_cols (расширено с 8 до 21)
2. ✅ **Расширен metadata блок** (добавлены risk_off_flag и fear_greed индикаторы)
3. ✅ **Увеличен EXT_NORM_DIM** для размещения дополнительных технических индикаторов

Возможные дальнейшие расширения:
- Добавить parkinson volatility в norm_cols
- Использовать sma_60 и ret_5m
- Добавить yang_zhang_720h (месячная волатильность)

Текущие **56 признаков** включают самые важные индикаторы для алготрейдинга!

---

**Last Updated**: 2025-11-11
**Status**: Исторический документ с описанием эволюции размера observation (57→43→56). Для актуальной структуры см. OBSERVATION_MAPPING.md
