...
| **Dukascopy adapter** (forex public bi5 tick-feed, keyless) | `adapters/dukascopy/market_data.py` | `pytest tests/test_dukascopy_adapter.py` |
| **Gas Guard** (on-chain gas oracle + порог + preflight) | `services/web3/gas_oracle.py` | `pytest tests/test_web3_custody.py::TestGasGuard` |
| **Fireblocks MPC** (реальный RS256-JWT API-клиент) | `services/custody/fireblocks_client.py` | `pytest tests/test_web3_custody.py::TestFireblocks` |
...
| **DAG research-пайплайн** (deps/resume/LeakGuard-пол) | `services/research_pipeline.py`, `configs/pipelines/` | `pytest tests/test_quant_gaps_p2him.py::TestResearchPipelineDAG` |
| **GPU-обучение** (детекция + --device) | `services/hardware.py`, `train_model_multi_patch.py` | `pytest tests/test_quant_gaps_p2him.py::TestHardware` |
| **Интрадей-фиды** (минутки/тики, entitlement) | `services/premium_data.py`, `scripts/download_premium_data.py` | `pytest tests/test_quant_gaps_p2him.py::TestPremiumData` |
| **Сравнение экспериментов** (compare/bundle) | `app.py` (`/api/experiments/*/compare`), Pro MLOps UI | `pytest tests/test_quant_gaps_p2him.py::TestExperimentCompare` |
...
|------|-------------|---------|
| **2026-07-16** | feat(custody): Fireblocks withdrawal/transfer flow (реальная отправка транзакций) | Клиент `services/custody/fireblocks_client.py` расширен: `estimate_fee`/`create_transaction`/`get_transaction` (реальный `POST /v1/transactions`, MPC co-signing на стороне Fireblocks), `validate_transfer` + `build_transfer_payload` (**amount строкой**, `externalTxId` идемпотентность). REST-оркестрация с safeguard'ами движения денег: **two-step ceremony** (`/withdraw/preview`→`/withdraw/submit`, одноразовый token, TTL 10 мин, anti-replay), **Gas Guard gate** на submit (свежий preflight, breach→409 fail-closed), долговечный журнал `state/fireblocks_withdrawals.jsonl`, `/tx/{id}` статус. UI: двухшаговая форма отправки + нативный confirm. 8 новых тестов (полный flow + create ровно 1 раз + gas-block + anti-replay), всего 26. См. [docs/WEB3_CUSTODY_FEATURES.md](docs/WEB3_CUSTODY_FEATURES.md) §4 |
| **2026-07-16** | feat(web3): Gas Guard + Fireblocks MPC + EIP-6963 — закрыты 3 honestly-disabled фичи Lite Portfolio реальными реализациями | **Gas Guard** `services/web3/gas_oracle.py`: реальный `eth_gasPrice` с публичного RPC (без ключей) + долговечный порог + armed/breached-вердикт + **fail-closed preflight** (`allow=False` → on-chain tx блокируется); REST `/api/web3/{gas,gas_guard,gas_guard/preflight}` + живая UI-панель. **Fireblocks** `services/custody/fireblocks_client.py`: **настоящий** клиент Fireblocks REST с RS256-JWT (claims uri/nonce/iat/exp/sub/bodyHash — проверено против pub-ключа), `test_connection`→`/v1/vault/accounts_paged`; приватный ключ хранится ТОЛЬКО по пути (не копируется); честный отказ без кред; REST `/api/custody/fireblocks/{status,connect,vaults}` + рабочая UI-форма. **WalletConnect→EIP-6963**: реальный multi-injected-wallet discovery (MetaMask/Rabby/Coinbase/Brave…, offline, без SDK). 18 тестов + реальный сетевой смоук (живой газ ETH/Base/ARB, breach→block). См. [docs/WEB3_CUSTODY_FEATURES.md](docs/WEB3_CUSTODY_FEATURES.md) |
| **2026-07-16** | feat(forex): Dukascopy market-data адаптер (публичный bi5 tick-feed) — закрыта заглушка, UI предлагал его для forex | `adapters/dukascopy/market_data.py` (`DukascopyMarketDataAdapter`, MarketDataAdapter): почасовые LZMA `.bi5`-файлы `datafeed.dukascopy.com` **без авторизации** (месяц 0-индексный!), 20-байт `>IIIff` записи, масштаб пунктов (FX 1e5 / JPY·металлы 1e3), тики→OHLCV ресемпл (pandas, bid/ask-каналы), get_tick/stream (polling); транспорт — plain `requests` (RestBudgetSession ломает бинарь). Data-only (как Yahoo), зарегистрирован MARKET_DATA для DUKASCOPY. Проводка: `scripts/download_forex_data.py --provider {oanda\|dukascopy}` + `download_pair_dukascopy` (та же схема) + `app.py` forex-ingest передаёт `--provider`; `services/premium_data.py` матрица + premium-vendor select; adapters status/test_connection. 13 тестов + реальный сетевой смоук (4995 тиков, 14 баров через ingest). См. [docs/DUKASCOPY_ADAPTER.md](docs/DUKASCOPY_ADAPTER.md) |
| **2026-07-16** | feat(quant): DAG-пайплайн + GPU-обучение + сравнение экспериментов + интрадей-фиды (P2-H/I/M, §5.21+/22/24/25) | **DAG**: `services/research_pipeline.py` (YAML-спеки `configs/pipelines/`, topo-Kahn, blocked fail-closed, durable resume, LeakGuard-пол 8000 в движке) + scheduler-action `pipeline.run` + REST `/api/pipeline/*` + карточка у планировщика. **GPU**: `services/hardware.py` (честная детекция torch-сборки/CUDA/VRAM + причина + install-hint) + `train_model_multi_patch.py --device` → env-мост → `DistributionalPPO(device=…)` + `/api/hardware/gpu` + GPU-чип в Quant Lab. **Сравнение**: `/api/experiments/{exp}/compare` (union+differs) + reproducibility-бандл `/runs/{id}/bundle` + экран в Pro MLOps (лучшее зелёным, отличия жёлтым). **Фиды**: `services/premium_data.py` (entitlement-матрица; минутки → `data/minute/*` в схеме download_stock_data + sha256-манифест; тик-бэкфилл Binance aggTrades fromId → `data/ticks/*`) + CLI + `/api/data/premium/*` + карточка в Data Manager. 25 тестов + регрессия 105 + реальный сетевой смоук (120 баров, 1834 тика). См. [docs/QUANT_GAPS_P2HIM_CLOSURE.md](docs/QUANT_GAPS_P2HIM_CLOSURE.md) |
